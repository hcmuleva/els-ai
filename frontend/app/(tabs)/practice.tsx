...
const STATUS_COLORS: Record<ClassroomItem['status'], string> = {
  // #16a34a only hits 3.3:1 as bold 11px text on white; darkened to match
  // Colors.success (6.5:1) since this value is used for statusBadgeText.
  active: '#176B47',
  completed: '#525C6B',
  draft: '#2563eb',
};
              ...
              {assignmentModal?.status === 'submitted' ? (
                <View style={{ backgroundColor: '#D6F5D6', borderRadius: 16, padding: 16, margin: 4, gap: 8 }}>
                  ...
                  {submissionAttachmentUrl ? (
                    <Text style={{ fontSize: 12, color: '#2D5DC9', fontWeight: '600' }}>📎 Attachment submitted</Text>
                  ) : null}
                </View>
              ) : (
                <>
