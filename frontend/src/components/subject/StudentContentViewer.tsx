            ...
            <View style={s.section} onLayout={(e) => { sectionYs.current[`s-${curIdx}`] = e.nativeEvent.layout.y; }}>
              ...
              {url && (content.contentType === 'audio' || isAudioUrl(url)) && !isVideoUrl(url) && !isYouTubeUrl(url) && (
                <View style={{ marginTop: 10 }}>
                  <AudioPlayer uri={url} title={content.title} subtitle={topic.subject} accentColor="#2D5DC9" bgColor="#D6EAFF" />
                </View>
              )}
                      ...
                      >
                        <View style={[s.moreCardIconWrap, { backgroundColor: active ? `${itemCfg.accent}20` : '#F0F0F8' }]}>
                          {yt
                            ...
                            : <itemCfg.Icon size={22} color={active ? itemCfg.accent : '#525C6B'} />
                          }
                        </View>
                        <LatexText content={item.title} style={s.moreCardTitle} compact compactHeight={36} numberOfLines={2} background="transparent" />
                        <Text style={[s.moreCardMeta, { color: active ? itemCfg.accent : '#525C6B' }]}>
