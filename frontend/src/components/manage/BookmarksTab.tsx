    ...
    <View style={c.root}>
      ...
      <View style={c.filterSection}>
        <View style={c.searchBar}>
          ...
          />
          {search !== '' && <Pressable onPress={() => setSearch('')}><X size={14} color="#9A9AB0" /></Pressable>}
        </View>
      ...
      <ScrollView contentContainerStyle={c.list} showsVerticalScrollIndicator={false}>
        {loading ? (
          <View style={c.emptyWrap}><ActivityIndicator size="large" color="#4A90E2" /></View>
        ) : bookmarks.length === 0 ? (
          <View style={c.emptyWrap}>
            <View style={c.emptyIcon}><Bookmark size={34} color="#4A90E2" /></View>
          ...
          bookmarks.map((b) => (
            <View key={b.id} style={c.card}>
              <View style={c.cardTop}>
                <View style={c.artBox}><BookmarkPlus size={24} color="#4A90E2" /></View>
                <View style={c.cardInfo}>
                  ...
                  <View style={c.cardChipRow}>
                    ...
                    {b.contentCount > 0 ? <View style={c.cardChip}><FileText size={10} color="#5A7AB0" /><Text style={c.cardChipText}>{b.contentCount}</Text></View> : null}
                    {b.quizCount > 0 ? <View style={c.cardChip}><Trophy size={10} color="#E65100" /><Text style={c.cardChipText}>{b.quizCount}</Text></View> : null}
                  </View>
                  {b.subjects.length > 0 ? (
                    <View style={c.cardChipRow}>
              ...
              <View style={c.cardFooter}>
                <Pressable style={[c.footerBtn, { backgroundColor: '#FFF3E0' }]} onPress={() => setEditing(b)}>
                  <Pencil size={13} color="#E65100" /><Text style={[c.footerBtnText, { color: '#E65100' }]}>Edit</Text>
                </Pressable>
                <Pressable style={[c.footerBtn, { backgroundColor: '#FEF0ED' }]} onPress={() => handleDelete(b.id)}>
                  <Trash2 size={13} color="#E05A3A" /><Text style={[c.footerBtnText, { color: '#E05A3A' }]}>Delete</Text>
                </Pressable>
              </View>
            </View>
          ))
                    ...
                    return (
                      <View key={topic.id} style={c.topicBlock}>
                        <Pressable style={c.topicRow} onPress={() => toggleExpand(topic.id)}>
                          {isOpenTopic ? <ChevronDown size={15} color="#4A90E2" /> : <ChevronRight size={15} color="#9A9AB0" />}
                    ...
                    <View key={subj} style={c.selGroup}>
                      ...
                      {items.map((it) => (
                        <View key={it.key} style={c.selRow}>
                          {it.itemType === 'content' ? <FileText size={12} color="#5A7AB0" /> : <Trophy size={12} color="#E65100" />}
                          <Text style={c.selTitle} numberOfLines={1}>{it.title}</Text>
                          <Pressable onPress={() => removeItem(it.key)} style={c.selRemove}><X size={13} color="#E05A3A" /></Pressable>
                        </View>
                      ))}
                    </View>
                  ))
...
const c = StyleSheet.create({
  ...
  selectChipText: { fontSize: 13, fontWeight: '600', color: '#9A9AB0' },
  ...
  removeAllText: { fontSize: 11, fontWeight: '700', color: '#E05A3A' },
  panelBody: { gap: 6 },
  panelHint: { fontSize: 12, color: '#9A9AB0', paddingVertical: 12, textAlign: 'center' },
  ...
  childRowSel: { borderColor: '#4A90E2', backgroundColor: '#EBF4FF' },
