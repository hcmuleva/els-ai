...
const TOPIC_ICON_COLORS = ['#2D5DC9', '#E91E8C', '#7DC67A', '#9B8EC4', '#E6A817', '#D33F13'];
...
const TOPIC_ICONS: { Icon: LucideIcon; color: string }[] = [
  { Icon: BookOpen,   color: '#2D5DC9' },
  { Icon: Palette,    color: '#A81762' },
  { Icon: Leaf,       color: '#2F6B2D' },
  { Icon: Telescope,  color: '#554E6C' },
  { Icon: Star,       color: '#8F680C' },
  { Icon: Music,      color: '#D33F13' },
];
...
const QUIZ_TYPE_STYLE: Record<string, QuizTypeCfg> = {
  practice:   { Icon: PenLine,      color: '#2D5DC9', bg: '#D6EAFF' },
  // Darkened from #D33F13 (3.96:1 on this bg, short of the 4.5:1 needed at this text size).
  exam:       { Icon: GraduationCap, color: '#B03A19', bg: '#FFE8D6' },
  homework:   { Icon: ClipboardList, color: '#554E6C', bg: '#EDE4FF' },
  assessment: { Icon: BarChart2,    color: '#2F6B2D', bg: '#D6F5D6' },
};
const DEFAULT_QT_STYLE: QuizTypeCfg = { Icon: Trophy, color: '#525C6B', bg: '#F4F4FB' };
...
const d = StyleSheet.create({
  ...
  // Darkened from #4CAF82 (2.5:1 on this bg) to clear WCAG AA; matches Colors.success.
  previewBtnText: { color: '#176B47', fontWeight: '800', fontSize: 12 },
  ...
  editBtn:  { backgroundColor: '#2D5DC9', borderRadius: 10, paddingHorizontal: 18, paddingVertical: 8 },
  ...
  // Dark (not light) overlay so white badge text keeps contrast against the
  // hero cover image regardless of its color (2.85:1 with the old light tint).
  heroBadge:            { backgroundColor: 'rgba(0,0,0,0.35)', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 3 },
  ...
  emptyText: { fontSize: 14, fontWeight: '700', color: '#525C6B' },
    ...
    <View style={s.card}>
      ...
      <View style={s.cardFooter}>
        ...
        <Pressable style={[s.footerBtn, { backgroundColor: '#FFF3E0' }]} onPress={() => onAction('edit')}>
          ...
          <Text style={[s.footerBtnText, { color: '#B23D00' }]}>Edit</Text>
        </Pressable>
        <Pressable style={[s.footerBtn, { backgroundColor: '#FEF0ED' }]} onPress={() => onAction('delete')}>
          <Trash2 size={13} color="#B03A19" />
          <Text style={[s.footerBtnText, { color: '#B03A19' }]}>Delete</Text>
        </Pressable>
      </View>
    </View>
  );
            ...
            loading ? (
              ...
              <View style={s.emptyWrap}>
                <View style={{ width: 72, height: 72, borderRadius: 20, backgroundColor: '#EEF4FF', alignItems: 'center', justifyContent: 'center' }}>
                  <FolderOpen size={36} color="#2D5DC9" />
                </View>
                    ...
                    <View key={qItem.id} style={s.sectionItem}>
                      ...
                      <View style={s.sectionItemActions}>
                        <TouchableOpacity onPress={() => setQuizIds((p) => moveUp(p, qidx))} disabled={qidx === 0} style={[s.orderBtn, qidx === 0 && { opacity: 0.2 }]}>
                          <ChevronUp size={14} color="#D33F13" />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => setQuizIds((p) => moveDown(p, qidx))} disabled={qidx === quizIds.length - 1} style={[s.orderBtn, qidx === quizIds.length - 1 && { opacity: 0.2 }]}>
                          <ChevronDown size={14} color="#D33F13" />
                        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  filterLabel:   { fontSize: 11, fontWeight: '800', color: '#525C6B', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 8 },
  ...
  applyBtn:      { borderRadius: 999, paddingHorizontal: 14, paddingVertical: 7, backgroundColor: '#2D5DC9' },
