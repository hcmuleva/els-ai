...
const SUBJECT_SYMBOL_ICONS: Record<string, IconEntry> = {
  'book-open': { Icon: BookOpen, color: '#2D5DC9', bg: '#D6EAFF' },
  hash: { Icon: Hash, color: '#D33F13', bg: '#FFE8D6' },
  ...
  globe: { Icon: Globe, color: '#D33F13', bg: '#FFE8D6' },
...
function getIconEntry(isSubject: boolean): IconEntry {
  if (isSubject) return { Icon: BookOpen, color: '#525C6B', bg: '#F0F0F8' };
  return { Icon: School, color: '#2D5DC9', bg: '#EBF4FF' };
}
...
const s = StyleSheet.create({
  ...
  itemTextActive: { color: '#2D5DC9', fontWeight: '700' },
});
