...
export default function MemoryMatchRenderer({ questionData, onComplete, theme, apiBase = '' }: Props) {
  ...
  const accentColor   = theme?.accent ?? '#2D5DC9';
  // Accuracy = pairs matched / total pairs (e.g. 2/4 = 50%)
  const accuracy      = Math.round((matched.size / usedPairs.length) * 100);
  const accuracyColor = accuracy >= 80 ? '#4CAF50' : accuracy >= 50 ? '#E6A020' : accuracy > 0 ? '#D33F13' : accentColor;
    ...
    <View style={mm.wrapper}>
      ...
      <View style={{ alignItems: 'center' }}>
        ...
        >
          ...
          {/* ── Banners ── */}
          {limitHit && (
            <View style={[mm.banner, { backgroundColor: '#FFF3E0', borderColor: '#D33F13' }]}>
              <Text style={[mm.bannerText, { color: '#B23D00' }]}>
...
const mm = StyleSheet.create({
  ...
  progressText: { fontSize: 10, fontWeight: '700', color: '#525C6B', textAlign: 'center' },
  ...
  clickMeterTitle:   { fontSize: 11, fontWeight: '800', color: '#525C6B', textTransform: 'uppercase', letterSpacing: 0.5 },
