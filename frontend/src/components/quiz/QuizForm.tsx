...
export type CreatedQuizSummary = {
  id: string;
  title: string;
  classLevel?: string;
  subject?: string;
  questionCount: number;
  attachedQuestions: number;
};
...
function inferQuizType(questions: QuizFormQuestion[]): string {
...
export type QuizFormProps = {
  apiFetch: QuizFormApiFetch;
  mode?: CreationMode;
  initialClassLevel?: string;
  initialSubject?: string;
  /** Question ids to mark as selected on mount (e.g. just-created questions). */
  extraSelectedIds?: string[];
  /** Increment this to force a question-bank reload. */
  refreshKey?: number;
...
export default function QuizForm({
  ...
  const [viewQuestion, setViewQuestion] = useState<QuizFormQuestion | null>(null);
  const [questionBank, setQuestionBank] = useState<QuizFormQuestion[]>([]);
  const [draft, setDraft] = useState<AssessmentDraft>({
    ...INITIAL_DRAFT,
    classLevel: initialClassLevel || '',
    subject: initialSubject || '',
  });
  const [selectedIds, setSelectedIds] = useState<string[]>(extraSelectedIds || []);
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [subjectCatalogItems, setSubjectCatalogItems] = useState<SubjectCatalogItem[]>([]);

  useEffect(() => {
    if (extraSelectedIds && extraSelectedIds.length) {
      setSelectedIds((prev) => {
        const set = new Set(prev);
        extraSelectedIds.forEach((id) => set.add(id));
        return Array.from(set);
      });
    }
  }, [extraSelectedIds?.join('|')]);

  const setCurrentDraft = (patch: Partial<AssessmentDraft>) => setDraft((c) => ({ ...c, ...patch }));
  ...
  const selectedQuestions = useMemo(
    () => selectedIds.map((id) => selectedQuestionMap.get(id)).filter((q): q is QuizFormQuestion => Boolean(q)),
    [selectedIds, selectedQuestionMap],
  );
  ...
  const createAssessment = async () => {
    ...
    try {
      const selected = selectedIds
        ...
        .filter((q): q is QuizFormQuestion => Boolean(q));
      ...
      for (const sourceQuestionId of selectedIds) {
      ...
      if (attachedQuestions === 0) throw new Error('Created successfully, but no selected questions were attached.');
      ...
      onCreated?.({
        ...
        classLevel: draft.classLevel || undefined,
        subject: draft.subject || undefined,
        questionCount: attachedQuestions,
        attachedQuestions,
      });
    ...
    <View style={s.wrapper}>
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        {!embedded && (
          <View style={s.pageHeader}>
            ...
            <View style={s.pageHeaderIcon}>
              {activeMode === 'quiz' ? <Trophy size={28} color="#2D5DC9" /> : <GraduationCap size={28} color="#9B8EC4" />}
            </View>
          </View>
        )}
        ...
        <View style={s.modeTabs}>
          ...
          >
            <Trophy size={16} color={activeMode === 'quiz' ? '#2D5DC9' : '#525C6B'} />
            <Text style={[s.modeTabText, activeMode === 'quiz' && s.modeTabTextActive]}>Quiz Mode</Text>
          </Pressable>
          <Pressable
            style={[s.modeTab, activeMode === 'exam' && s.modeTabExam]}
            onPress={() => { setActiveMode('exam'); setBankTab('question'); }}
          >
            <GraduationCap size={16} color={activeMode === 'exam' ? '#9B8EC4' : '#525C6B'} />
            <Text style={[s.modeTabText, activeMode === 'exam' && s.modeTabTextExam]}>Exam Mode</Text>
          </Pressable>
        </View>
        ...
        <View style={s.card}>
          <View style={s.cardHeaderRow}>
            <View style={[s.cardHeaderIcon, { backgroundColor: activeMode === 'quiz' ? '#D6EAFF' : '#EDE4FF' }]}>
              {activeMode === 'quiz' ? <PenLine size={18} color="#2D5DC9" /> : <BookOpen size={18} color="#9B8EC4" />}
            </View>
            <View>
          ...
          <View style={s.row}>
            <View style={[s.fieldGroup, { flex: 1 }]}>
              ...
              <Pressable style={s.dropdownField} onPress={() => setSelectorField('classLevel')}>
                <Text style={draft.classLevel ? s.dropdownText : s.dropdownPlaceholder}>
            ...
            <View style={[s.fieldGroup, { flex: 1 }]}>
              ...
              <Pressable
                style={[s.dropdownField, !draft.classLevel && s.dropdownFieldDisabled]}
                onPress={() => { if (draft.classLevel) setSelectorField('subject'); }}
                disabled={!draft.classLevel}
              >
                <Text style={draft.subject ? s.dropdownText : s.dropdownPlaceholder}>
                  {draft.subject || (draft.classLevel ? 'Select Subject' : 'Select class first')}
                </Text>
                <ChevronDown size={14} color="#525C6B" />
              </Pressable>
            </View>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.chipsRow}>
              {DIFFICULTIES.map((diff) => {
                ...
                const isActive = draft.difficultyLevel === diff;
                return (
                  <Pressable
                    key={diff}
          ...
          <Pressable
            style={s.checkboxRow}
            onPress={() => setCurrentDraft({ hasTimeLimit: !draft.hasTimeLimit })}
          >
            <View style={[s.checkbox, draft.hasTimeLimit && s.checkboxChecked]}>
              {draft.hasTimeLimit && <Check size={11} color="#fff" />}
            </View>
                  ...
                  <View key={`${bankTab}-${question.id}`} style={[s.questionCard, isSelected && s.questionCardSelected]}>
                    <View style={s.questionMeta}>
                      ...
                      <View style={[s.questionBadge, s.typeBadge]}>
                        <QuestionTypeIcon type={question.question_type} size={10} color="#2D5DC9" />
                        <Text style={[s.questionBadgeText, { color: '#2D5DC9' }]}>{typeLabel}</Text>
                      </View>
                    </View>
                    <LatexText
          ...
          {rowsToRender.length > PAGE_SIZE && (
            <View style={s.paginationBar}>
              ...
              >
                ...
                <ChevronRight size={16} color={bankPage >= totalPages - 1 ? '#C0C8D8' : '#2D5DC9'} />
              </Pressable>
            </View>
          )}
        </View>
      ...
      <SelectorModal
        ...
        selected={selectorField === 'classLevel' ? draft.classLevel : draft.subject}
          ...
          <View style={s.modalCard}>
            ...
            <ScrollView style={s.modalList}>
              ...
              <View style={s.previewMetaRow}>
                ...
                <View style={[s.questionBadge, s.typeBadge]}>
                  <Text style={[s.questionBadgeText, { color: '#2D5DC9' }]}>
      ...
      {message && (
        <View pointerEvents="none" style={[s.toast, message.type === 'success' ? s.toastSuccess : s.toastError]}>
...
const s = StyleSheet.create({
  wrapper: { flex: 1, backgroundColor: '#F5F7FF' },
  ...
  pageHeaderIcon: { width: 52, height: 52, borderRadius: 16, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' },
  pageTitle:      { fontSize: 22, fontWeight: '900', color: '#1a1a2e' },
  pageSubtitle:   { fontSize: 12, color: '#525C6B', fontWeight: '500', marginTop: 2 },

  modeTabs: { flexDirection: 'row', gap: 10 },
  modeTab: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, backgroundColor: '#fff', borderRadius: 14, borderWidth: 1.5, borderColor: '#E8ECF8', paddingVertical: 11 },
  modeTabActive: { borderColor: '#93c5fd', backgroundColor: '#EBF4FF' },
  modeTabExam:   { borderColor: '#C4B0E8', backgroundColor: '#F0EAFF' },
  modeTabText:        { fontSize: 13, fontWeight: '700', color: '#525C6B' },
  modeTabTextActive:  { color: '#2D5DC9' },
  modeTabTextExam:    { color: '#9B8EC4' },

  card: { backgroundColor: '#fff', borderRadius: 20, padding: 18, gap: 14 },
  ...
  cardSubtitle: { fontSize: 11, color: '#525C6B', fontWeight: '500', marginTop: 1 },
  ...
  explnChipActive:   { borderColor: '#2D5DC9', backgroundColor: '#EBF4FF' },
  explnChipText:     { fontSize: 12, fontWeight: '700', color: '#525C6B', textAlign: 'center' },
  explnChipTextActive:{ color: '#2D5DC9' },
  explnHint:         { fontSize: 11, color: '#525C6B', fontStyle: 'italic', marginTop: 2 },
  ...
  createBtn:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#2D5DC9', borderRadius: 14, paddingVertical: 14 },
  ...
  typeFilterChipActive: { borderColor: '#2D5DC9', backgroundColor: '#EBF4FF' },
  typeFilterChipText: { fontSize: 11, fontWeight: '700', color: '#525C6B' },
  typeFilterChipTextActive: { color: '#2D5DC9' },
  ...
  bankTabText:      { fontSize: 12, fontWeight: '700', color: '#525C6B' },
  bankTabTextActive:{ color: '#1a1a2e' },

  bankEmpty:     { alignItems: 'center', paddingVertical: 40, gap: 10 },
  bankEmptyText: { fontSize: 13, color: '#525C6B', textAlign: 'center' },
  questionList:  { gap: 10 },

  questionCard: { backgroundColor: '#F8F9FF', borderRadius: 14, padding: 14, gap: 8, borderWidth: 1.5, borderColor: '#E8ECF8' },
  ...
  previewFieldLabel:{ fontSize: 11, fontWeight: '800', color: '#525C6B', textTransform: 'uppercase', marginTop: 10, marginBottom: 4 },
