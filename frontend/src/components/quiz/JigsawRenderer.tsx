...
export default function JigsawRenderer({
  ...
  // accent on its own 13%-alpha tint (the "Grid" badge bg below) only reaches ~2.4:1;
  // theme.textColor is tuned for tinted-bg text and passes comfortably.
  const badgeTextColor = theme?.textColor ?? accent;
        ...
        >
          ...
          <View style={s.statsRow}>
            <View style={[s.badge, { backgroundColor: accent + '22' }]}>
              <Text style={[s.badgeVal, { color: badgeTextColor }]}>{gridSize}</Text>
          ...
          <View style={s.instructRow}>
            {selectedPiece !== null
              ? <Text style={[s.instr, { color: badgeTextColor }]}>Piece #{selectedPiece + 1} selected — tap a grid slot</Text>
          ...
          <View style={[s.section, { paddingHorizontal: HPAD }]}>
            ...
            {trayPieces.length === 0 && gameState === 'in_progress' ? (
              <View style={s.trayEmpty}>
                <Text style={[s.trayEmptyText, { color: lockedSlots.size === total ? '#15803D' : badgeTextColor }]}>
...
const s = StyleSheet.create({
  ...
  emptyText: { fontSize: 13, color: '#4E5D71', fontWeight: '600' },
  ...
  badgeKey: { fontSize: 9, fontWeight: '600', color: '#4E5D71', letterSpacing: 0.6, textTransform: 'uppercase', marginTop: 1 },

  instructRow: { minHeight: 20, alignItems: 'center', paddingHorizontal: 12 },
  instr: { fontSize: 11, color: '#4B5768', fontWeight: '600', textAlign: 'center' },

  section:  { gap: 8 },
  secLabel: { fontSize: 10, fontWeight: '900', color: '#4E5D71', letterSpacing: 1, textTransform: 'uppercase' },
  ...
  trayNum:       { position: 'absolute', bottom: 1, right: 2, fontSize: 7, color: '#4E5D71', fontWeight: '700' },
  ...
  endSub:   { fontSize: 12, color: '#4B5768', fontWeight: '600' },
});
