...
export type QuestionTheme = {
  ...
  emoji: string;
  label: string;
  // Foreground for text rendered directly on an `accent`-tinted (e.g. accent+alpha) chip/badge
  // background — `accent` itself doesn't reliably hit 4.5:1 on its own tint for every theme.
  badgeTextColor?: string;
};

const QUESTION_THEMES: Record<string, QuestionTheme> = {
  true_false: {
    bg: '#D6EAFF', cardBg: '#EBF4FF', accent: '#2D5DC9',
          ...
          <ScrollView contentContainerStyle={[styles.resultScroll, { paddingBottom: Math.max(insets.bottom + 24, 48) }]} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.statsCard}>
              <View style={[styles.statCell, { backgroundColor: '#F0F4FF' }]}>
                <Text style={[styles.statValue, { color: '#2D5DC9' }]}>
...
const styles = StyleSheet.create({
  ...
  loadingCard: {
    ...
    shadowColor: '#2D5DC9', shadowOpacity: 0.12, shadowOffset: { width: 0, height: 6 },
  ...
  introTagline: {
    fontSize: 12, fontWeight: '800', color: '#2D5DC9',
  ...
  xpChip: {
    backgroundColor: '#2D5DC9', paddingHorizontal: 12, paddingVertical: 6,
  ...
  dotActive: { width: 24, height: 8, borderRadius: 4, backgroundColor: '#2D5DC9' },
  ...
  questionTypePillText: { fontSize: 11, fontWeight: '800', color: '#2D5DC9' },
  questionTitle: {
  ...
  xpBarLabel: { fontSize: 11, fontWeight: '800', color: '#2D5DC9' },
  ...
  explnLabel: {
    fontSize: 11, fontWeight: '800', color: '#2D5DC9',
    textTransform: 'uppercase', letterSpacing: 0.6,
  },
  explnScroll: { maxHeight: 220 },
  explnText: { fontSize: 14, color: '#334155', lineHeight: 21 },
  explnNextBtn: {
    backgroundColor: '#2D5DC9', borderRadius: 999, paddingVertical: 14, alignItems: 'center',
    shadowColor: '#2D5DC9', shadowOpacity: 0.32, shadowOffset: { width: 0, height: 5 },
    shadowRadius: 12, elevation: 4,
  },
