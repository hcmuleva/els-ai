      ...
      {activeSection === 'conversations' && (
        <View style={{ gap: 10, paddingHorizontal: 16, paddingTop: 4 }}>
          <Pressable style={s.newThreadBtn} onPress={openNewThread}>
            <View style={s.newThreadIcon}><Plus size={16} color="#4A90E2" /></View>
            <Text style={s.newThreadBtnText}>New Feedback</Text>
            <ChevronRight size={14} color="#4A90E2" />
          </Pressable>
            ...
            threads.map((thread) => (
              <Pressable key={thread.id} style={s.threadCard} onPress={() => openThread(thread.id)}>
                <View style={[s.threadIconBox, { backgroundColor: thread.unreadCount > 0 ? '#D6EAFF' : '#F0F4FF' }]}>
                  <MessageCircle size={20} color={thread.unreadCount > 0 ? '#4A90E2' : '#9A9AB0'} />
                </View>
                <View style={s.threadInfo}>
                  <View style={{ flexDirection: 'row', gap: 5, flexWrap: 'wrap', marginBottom: 2 }}>
                    {thread.category && (
                      <View style={[s.tagBadge, { backgroundColor: thread.category === 'academic' ? '#E3F2FD' : '#FFF3E0' }]}>
                        <Text style={[s.tagBadgeText, { color: thread.category === 'academic' ? '#1976D2' : '#E65100' }]}>
                  ...
                  <View style={s.threadMetaRow}>
                    <Clock size={10} color="#9A9AB0" />
                    <Text style={s.threadTime}>{thread.lastMessageAt ? timeAgo(thread.lastMessageAt) : formatDate(thread.createdAt)}</Text>
                    <View style={[s.statusDot, { backgroundColor: thread.status === 'open' ? '#4CAF50' : '#9A9AB0' }]} />
                    <Text style={{ fontSize: 10, color: thread.status === 'open' ? '#4CAF50' : '#9A9AB0', fontWeight: '600' }}>
      ...
      <Modal visible={!!threadDetail || threadLoading} transparent animationType="slide" onRequestClose={() => setThreadDetail(null)}>
        <View style={s.modalOverlay}>
          <View style={s.modalSheet}>
            {threadLoading ? (
              <View style={s.loaderWrap}><ActivityIndicator size="large" color="#4A90E2" /></View>
            ) : threadDetail ? (
              <>
              ...
              <View style={{ zIndex: 10, position: 'relative' }}>
                ...
                {selectedSubjectTitle && !subjectDropdownOpen ? (
                  ...
                  >
                    ...
                    <Text style={{ fontSize: 11, color: '#4A90E2' }}>Change</Text>
                  </Pressable>
                ) : (
                  <View>
                    <TextInput
                      ...
                      <View style={s.dropdownList}>
                        <ScrollView style={{ maxHeight: 220 }} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
                          ...
                          {(newCategory === 'academic' ? subjectsList.academic : subjectsList.nonAcademic)
                            ...
                            <Text style={{ padding: 12, fontSize: 12, color: '#9A9AB0', textAlign: 'center' }}>No subjects found</Text>
                          )}
                        </ScrollView>
                      </View>
                    )}
...
const s = StyleSheet.create({
  ...
  sectionTabText: { fontSize: 11, fontWeight: '700', color: '#9A9AB0' },
  ...
  newThreadBtnText: { flex: 1, fontSize: 14, fontWeight: '700', color: '#4A90E2' },
  ...
  remarkMeta: { fontSize: 11, color: '#9A9AB0', fontWeight: '600', marginTop: 1 },
  parentNoteBox: { backgroundColor: '#F0F7FF', borderRadius: 12, padding: 10, borderLeftWidth: 3, borderLeftColor: '#4A90E2' },
  parentNoteLabel: { fontSize: 10, fontWeight: '800', color: '#4A90E2', marginBottom: 3, textTransform: 'uppercase', letterSpacing: 0.4 },
  ...
  submitDesc: { fontSize: 12, color: '#9A9AB0', lineHeight: 18 },
  ...
  modalSub: { fontSize: 11, color: '#9A9AB0', fontWeight: '500' },
  ...
  replyBtn: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#4A90E2', alignItems: 'center', justifyContent: 'center' },
  ...
  catChipActive: { backgroundColor: '#4A90E2', borderColor: '#4A90E2' },
  ...
  dropdownItemTextActive: { color: '#4A90E2', fontWeight: '700' },
});
