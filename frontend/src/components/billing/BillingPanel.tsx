...
export function BillingPanel({ mode, organizations, currentOrganizationId, selectedOrgId, onSelectOrg }: Props) {
  ...
  const subStatusColor = (status?: string) => {
    ...
    // Colors.warning/Colors.error on the white statCard are 2.06:1 / 3.76:1,
    // both under 4.5:1; darkened for this text-only usage.
    if (status === 'past_due') return '#8F4A17';
    return '#B71C1C';
  };
  ...
  return (
    <View style={styles.root}>
        ...
        <View style={styles.card}>
          ...
          <Pressable style={styles.orgSelector} onPress={() => setOrgPickerOpen(true)}>
            <View style={styles.orgSelectorIcon}>
              {(activeOrg as any)?.logoUrl || (activeOrg as any)?.logo ? (
                <Image source={{ uri: (activeOrg as any).logoUrl || (activeOrg as any).logo || '' }} style={styles.orgSelectorImage} />
              ) : (
            ...
            <View style={styles.orgSelectorMeta}>
              ...
              <Text style={styles.orgSelectorSub} numberOfLines={1}>
                {activeOrg?.subdomain || `${organizations.length} organizations`}
              </Text>
            </View>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <View style={[styles.pill, { backgroundColor: Colors.warningLight }]}>
              {/* Colors.warning on warningLight is 1.84:1; darkened for text-only use */}
              <Text style={[styles.pillText, { color: '#8F4A17' }]}>{pendingInvoices.length}</Text>
            </View>
          </View>
      ...
      <SelectorModal
        ...
        showAny={false}
        onSelect={(value) => onSelectOrg && onSelectOrg(value)}
