...
const CATEGORY_CONFIG: Record<string, { icon: any; bg: string; color: string }> = {
  classroom:   { icon: GraduationCap, bg: '#D6EAFF', color: Colors.primary },
  remark:      { icon: Megaphone,     bg: '#EDE4FF', color: Colors.purple },
  billing:     { icon: Receipt,       bg: '#FFE8D6', color: Colors.accent },
  assignment:  { icon: ClipboardList, bg: '#D6EAFF', color: Colors.primary },
  quiz:        { icon: BookOpen,      bg: '#FFE8D6', color: Colors.accent },
  achievement: { icon: Trophy,        bg: '#FFF5CC', color: '#E6A817' },
  system:      { icon: Megaphone,     bg: '#EDE4FF', color: Colors.purple },
};
...
export function NotificationBell() {
  ...
  return (
    <View style={s.wrapper}>
      <Pressable onPress={() => setOpen(true)} style={s.bellBtn} accessibilityRole="button" accessibilityLabel="Notifications">
      ...
      <View style={[p.container, { paddingTop: Math.max(insets.top, 8) }]}>
        ...
        <View style={p.header}>
          ...
          <View style={p.headerRight}>
            {unreadCount > 0 ? (
              <TouchableOpacity onPress={onMarkAllRead} style={p.markAllBtn} accessibilityRole="button" accessibilityLabel="Mark all read">
                <CheckCheck size={14} color={Colors.primary} />
                <Text style={p.markAllText}>Mark all read</Text>
              </TouchableOpacity>
            ) : null}
            {readCount > 0 ? (
              <TouchableOpacity onPress={onDeleteAllRead} style={p.markAllBtn} accessibilityRole="button" accessibilityLabel="Delete read notifications">
                <Trash2 size={14} color={Colors.error} />
                <Text style={p.deleteAllText}>Delete read</Text>
              </TouchableOpacity>
            ) : null}
            <Pressable onPress={onClose} style={p.closeBtn} accessibilityRole="button" accessibilityLabel="Close notifications">
...
const p = StyleSheet.create({
  ...
  deleteAllText: { fontSize: 12, fontWeight: '700', color: Colors.error },
  closeBtn: {
  ...
  cancelledPillText: { fontSize: 11, fontWeight: '700', color: '#525C6B' },
  liveBadge: {
  ...
  joinButtonLive: { backgroundColor: Colors.error },
  joinButtonNormal: { backgroundColor: Colors.primary },
  ...
  itemMessage: { fontSize: 12, color: '#525C6B', lineHeight: 16 },
  itemFooter: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 },
  // Darkened from #B0B8CC (1.99:1 on white) to clear WCAG AA.
  itemTime: { fontSize: 11, color: '#525C6B' },
  ctaPill: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 999, backgroundColor: '#EAF1FB' },
  ctaPillText: { fontSize: 11, fontWeight: '700', color: Colors.primary },
