...
function GrowthChip({ delta }: { delta: number }) {
  ...
  return (
    <View style={[a.growthChip, { backgroundColor: `${color}1A` }]}>
      ...
      <Text style={[a.growthChipText, { color }]}>{`${up ? '+' : ''}${delta}%`}</Text>
    </View>
  );
}
...
function RiskPill({ level }: { level: RiskLevel }) {
  ...
  return (
    <View style={[a.growthChip, { backgroundColor: `${clr}1A` }]}>
      ...
      <Text style={[a.growthChipText, { color: clr }]}>{level}</Text>
    </View>
  );
}
...
const a = StyleSheet.create({
  ...
  execItemLabel: { fontSize: 9.5, fontWeight: '700', color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: 0.3 },
  execItemValue: { fontSize: 14, fontWeight: '900', color: '#fff', marginTop: 3 },
  execText: { fontSize: 12, color: 'rgba(255,255,255,0.85)', lineHeight: 19, marginTop: 8 },
