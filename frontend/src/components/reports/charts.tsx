        ...
        return (
          <Svg width={w} height={height}>
            {ticks.map((t) => {
              ...
              return (
                <G key={t}>
                  ...
                  <SvgText x={0} y={y + 3} fontSize={9} fill="#B0B0C8" fontWeight="600">
                    {t}
                    {yUnit}
                  </SvgText>
                </G>
            ...
            {labels.map((lb, i) => {
              ...
              return (
                <SvgText key={i} x={xFor(i)} y={height - 6} fontSize={8.5} fill="#9A9AB0" fontWeight="700" textAnchor="middle">
                  {lb}
                </SvgText>
              );
            })}
  ...
  return (
    <View style={{ paddingTop: 4 }}>
      <View style={{ flexDirection: 'row' }}>
        <View style={{ width: 28, height, justifyContent: 'space-between', alignItems: 'flex-end', paddingRight: 6 }}>
          {[1, 0.75, 0.5, 0.25, 0].map((f) => (
            <Text key={f} style={{ fontSize: 9, color: '#B0B0C8', fontWeight: '600' }}>
              {round(niceMax * f)}
              {unit}
            </Text>
          ))}
      ...
      <View style={{ flexDirection: 'row', marginLeft: 28, marginTop: 6, gap: 6 }}>
        {labels.map((lb, i) => (
          <Text key={i} style={{ flex: 1, textAlign: 'center', fontSize: 8.5, color: '#9A9AB0', fontWeight: '700' }}>
            {lb}
          </Text>
        ))}
      </View>
...
const RISK_CLR: Record<string, { bg: string; fg: string }> = {
  Low: { bg: '#D4EFE3', fg: '#1F8A5B' },
  Medium: { bg: '#FFF0DC', fg: '#C77B2B' },
  High: { bg: '#FEE2E2', fg: '#D63A3A' },
};
