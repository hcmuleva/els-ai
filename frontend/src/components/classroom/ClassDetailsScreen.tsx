...
// ── Mini bar chart (SVG) ──────────────────────────────────────────────────────
function MiniBarChart({ data, color = '#2D5DC9', maxVal = 5 }: {
      ...
      {data.map((d, i) => {
        ...
        return (
          <G key={i}>
            ...
            <SvgText x={x + BAR_W / 2} y={H - PAD + 14} fontSize={9} textAnchor="middle" fill="#525C6B">{d.label.slice(0, 6)}</SvgText>
...
function StudentRow({ student, totalAssignments, totalQuizzes, onRemark, onQuizDetails, onAssignmentDetails }: {
  ...
  const completionColor = completionPct === 100 ? '#7DC67A' : completionPct >= 50 ? '#2D5DC9' : '#D33F13';
  ...
  return (
    <View style={ds.studentCard}>
      ...
      <View style={ds.chipRow}>
        {totalQuizzes > 0 && (
          <View style={[ds.statusChip, { backgroundColor: quizOk ? '#D6F5D6' : '#FFF3E0', borderColor: quizOk ? '#7DC67A40' : '#D33F1340' }]}>
            {quizOk
              ? <CheckCircle size={11} color="#1A6B1A" />
              : <AlertCircle size={11} color="#B23D00" />}
            <Text style={[ds.statusChipText, { color: quizOk ? '#1A6B1A' : '#B23D00' }]}>
              {quizOk ? 'All Quizzes' : `Quiz ${student.quizzesCompleted}/${totalQuizzes}`}
            </Text>
          </View>
        )}
        {totalAssignments > 0 && (
          <View style={[ds.statusChip, { backgroundColor: asnOk ? '#D6F5D6' : '#FFF3E0', borderColor: asnOk ? '#7DC67A40' : '#D33F1340' }]}>
            {asnOk
              ? <CheckCheck size={11} color="#1A6B1A" />
              : <AlertCircle size={11} color="#B23D00" />}
            <Text style={[ds.statusChipText, { color: asnOk ? '#1A6B1A' : '#B23D00' }]}>
              {asnOk ? 'All Tasks' : `Task ${student.assignmentsSubmitted}/${totalAssignments}`}
            </Text>
          </View>
        )}
        {student.avgScore > 0 && (
          <View style={[ds.statusChip, { backgroundColor: '#D6EAFF', borderColor: '#2D5DC940' }]}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
          <View style={{ flexDirection: 'row', gap: 6 }}>
            {uniqueAchievs.slice(0, 6).map((a: any) => (
              <View key={a.id} style={[ds.achievChip, { backgroundColor: `${a.color ?? '#2D5DC9'}18` }]}>
                <Text style={{ fontSize: 14 }}>{a.emoji}</Text>
                {(a.count ?? 1) > 1 && <Text style={[ds.achievCount, { color: a.color ?? '#2D5DC9' }]}>×{a.count}</Text>}
              </View>
            ))}
...
export default function ClassDetailsScreen({ classroomId, apiFetch, onClose, onUploadMedia }: Props) {
  ...
  const renderAttemptAnswer = (qa: StudentQuizAttempt['questionAttempts'][number], index: number) => {
              ...
              {boardRows.map((row, rIdx) => (
                <View key={rIdx} style={{ flexDirection: 'row', gap: 8 }}>
                  {row.map((pair) => {
                    ...
                    return (
                      <View key={pair.id} style={[ds.mmCard, { flex: 1, backgroundColor: isOk ? '#E8F5E9' : '#FFF3F0', borderColor: isOk ? '#4CAF50' : '#D33F13' }]}>
                        {imgUrl
    ...
    // ── Jigsaw Puzzle ────────────────────────────────────────────────────────
    if (normalizedType === 'jigsaw' || normalizedType === 'jigsaw_puzzle') {
      ...
      const gridSize    = (rd.gridSize as string) || (questionData.gridSize as string) || '3x3';
      const difficulty  = (rd.difficulty as string) || (questionData.difficulty as string) || 'medium';
      ...
      const imageUrl    = resolveMediaUrl((questionData.image as string) || (questionData.prompt_image as string));
      const slotArr     = Array.isArray(rd.slotArrangement) ? (rd.slotArrangement as Array<number | null>) : null;
      const CELL        = 60;
      const GAP         = 2;
      return (
        <View key={`${qa.questionId}-${index}`} style={ds.answerCard}>
          <View style={[ds.answerCardBanner, { backgroundColor: '#E0F2FE' }]}>
            <Text style={[ds.answerCardBannerTitle, { color: '#0C4A6E' }]}>🧩 Jigsaw Puzzle</Text>
            <Text style={[ds.answerCardBadge, { backgroundColor: completed ? '#0EA5E9' : '#FF5252' }]}>
              {completed ? 'Completed' : 'Not finished'}
            </Text>
          </View>
          <View style={[ds.answerRow, { gap: 10 }]}>
            <Text style={ds.answerTitle}>{qa.questionTitle || qa.questionInstruction || 'Jigsaw Puzzle'}</Text>
            {/* Stats chips */}
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
              <View style={[ds.mmChip, { backgroundColor: '#E0F2FE' }]}>
                <Text style={[ds.mmChipTxt, { color: '#0369A1' }]}>Grid {gridSize} · {total} pieces</Text>
              </View>
              <View style={[ds.mmChip, { backgroundColor: diffBg }]}>
                <Text style={[ds.mmChipTxt, { color: diffColor }]}>{difficulty}</Text>
              </View>
              <View style={[ds.mmChip, { backgroundColor: '#F1F5F9' }]}>
                <Text style={[ds.mmChipTxt, { color: '#334155' }]}>{moves}{clickLim ? `/${clickLim}` : ''} moves</Text>
              </View>
              {timeTaken > 0 && (
                <View style={[ds.mmChip, { backgroundColor: '#F1F5F9' }]}>
                  <Text style={[ds.mmChipTxt, { color: '#334155' }]}>{timeTaken}s</Text>
                </View>
              )}
            </View>
            {/* Result bar */}
            <View style={{ gap: 4 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={ds.mmBarLabel}>{completed ? 'COMPLETED' : 'PROGRESS'}</Text>
                <Text style={[ds.mmBarLabel, { color: barColor }]}>{completed ? '✓ Done' : '✗ Incomplete'}</Text>
              </View>
              <View style={{ height: 8, backgroundColor: '#F0F0F5', borderRadius: 4, overflow: 'hidden' }}>
                <View style={{ height: 8, width: completed ? ('100%' as any) : ('30%' as any), backgroundColor: barColor, borderRadius: 4 }} />
              </View>
            </View>
            {/* Final answer grid preview */}
            {imageUrl && slotArr ? (
              <View style={{ gap: 6 }}>
                <Text style={ds.mmBoardLabel}>FINAL ANSWER</Text>
                <View style={{ gap: GAP }}>
                        ...
                        return (
                          <View
                            ...
                            style={{
                              ...
                              overflow: 'hidden',
                              borderWidth: 2,
                              borderColor: isEmpty ? '#CBD5E1' : isCorrect ? '#4CAF50' : '#D33F13',
                              backgroundColor: isEmpty ? '#F0F4FF' : undefined,
                            ...
                            {!isEmpty ? (
                              <Image
                                ...
                                style={{
                                  ...
                                  position: 'absolute',
                                  left: -((piece! % n) * CELL),
                                  top: -(Math.floor(piece! / n) * CELL),
                                }}
                              />
                            ) : (
                              <Text style={{ fontSize: 10, color: '#4E5D71', fontWeight: '700' }}>{slot + 1}</Text>
                            )}
                            {/* Correct/wrong badge */}
                            {!isEmpty && (
                              <View style={{ position: 'absolute', bottom: 2, right: 2, width: 12, height: 12, borderRadius: 6, backgroundColor: isCorrect ? '#4CAF50' : '#D33F13', alignItems: 'center', justifyContent: 'center' }}>
                                <Text style={{ fontSize: 7, color: '#fff', fontWeight: '900' }}>{isCorrect ? '✓' : '✗'}</Text>
                              </View>
                            )}
                          </View>
                        );
              ...
              <View style={{ gap: 6 }}>
                <Text style={ds.mmBoardLabel}>FINAL ANSWER</Text>
                <Image source={{ uri: imageUrl }} style={{ width: '100%', height: 160, borderRadius: 10 }} resizeMode="contain" />
              </View>
            ) : null}
          </View>
        </View>
      ...
      return (
        <View key={`${qa.questionId}-${index}`} style={ds.answerCard}>
          ...
          <View style={[ds.answerRow, { gap: 8 }]}>
            ...
            <View style={[ds.mmSentenceBox, { borderColor: isOk ? '#4CAF50' : '#D33F13' }]}>
      ...
      <View style={ds.screen}>
        ...
        <View style={[ds.header, { paddingTop: Math.max(insets.top, 12) }]}>
          ...
          <View style={[ds.statusPill, { backgroundColor: isLive ? '#D6F5D6' : '#F0F0F8' }]}>
            <View style={[ds.statusDot, { backgroundColor: isLive ? '#7DC67A' : '#525C6B' }]} />
          ...
          {TABS.map(([t, TabIcon, label]) => {
            ...
            return (
              <Pressable key={t} style={[ds.tab, active && ds.tabActive]} onPress={() => setTab(t)}>
                <TabIcon size={14} color={active ? '#2D5DC9' : '#525C6B'} />
            ...
            {tab === 'overview' && (
              <>
                ...
                <View style={ds.metricsGrid}>
                  ...
                  <MetricCard Icon={ClipboardList} value={summary?.assignmentDoneCount ?? 0}  label="Tasks Done"      color="#B23D00" bg="#FFF3E0" />
                ...
                <View style={ds.section}>
                  ...
                  <View style={ds.contentRow}>
                    ...
                    <View style={[ds.contentItem, { borderColor: '#FFE8D6' }]}>
                      <Trophy size={22} color="#D33F13" />
                      <Text style={[ds.contentVal, { color: '#B23D00' }]}>{classroom?.totalQuizzes ?? 0}</Text>
                  ...
                  <View style={ds.section}>
                    ...
                    <View style={ds.donutRow}>
                      <DonutChart
                        ...
                        color="#2D5DC9" label="Quiz Done"
                      />
                      <DonutChart
                        pct={Math.round((summary.assignmentDoneCount / summary.joinedCount) * 100)}
                        color="#7DC67A" label="Tasks Done"
                      />
                      <DonutChart pct={summary.completionPct} color="#D33F13" label="All Complete" />
                    </View>
                  </View>
                )}
              </>
            ...
            {tab === 'analytics' && (
              <>
                {students.length === 0 ? (
                  <View style={ds.emptyWrap}>
                    <View style={ds.emptyIconBox}>
                      <BarChart2 size={36} color="#525C6B" />
                    </View>
                        ...
                        {students.map((s, i) => {
                          ...
                          return (
                            <View key={s.studentId} style={[ds.tableRow, i % 2 === 0 && ds.tableRowAlt]}>
                              ...
                              <Text style={[ds.tableCell, { color: s.quizzesCompleted > 0 ? '#1A6B1A' : '#B23D00' }]}>{s.quizzesCompleted}</Text>
                              <Text style={[ds.tableCell, { color: s.assignmentsSubmitted > 0 ? '#1A6B1A' : '#B23D00' }]}>{s.assignmentsSubmitted}</Text>
                              <Text style={[ds.tableCell, { color: '#2D5DC9', fontWeight: '800' }]}>
                                {s.remark?.scorePerformance ? `${s.remark.scorePerformance}/5` : '–'}
                              </Text>
                              <Text style={[ds.tableCell, { color: pct === 100 ? '#1A6B1A' : '#B23D00', fontWeight: '800' }]}>{pct}%</Text>
                            </View>
                          );
                        })}
                      </View>
            ...
            <ScrollView contentContainerStyle={ds.content} showsVerticalScrollIndicator={false}>
              {detailTab === 'parentFeedback' ? (
                parentFeedbackLoading ? (
                  ...
                  <View style={ds.emptyWrap}>
                    <View style={ds.emptyIconBox}>
                      <MessageCircle size={36} color="#525C6B" />
                    </View>
                  ...
                  parentFeedbackItems.map((item) => (
                    <View key={item.id} style={ds.detailCard}>
                      ...
                      <Text style={{ fontSize: 10, color: '#525C6B', marginTop: 6 }}>
                ...
                (studentDetails?.assignments?.length ?? 0) === 0 ? (
                  <View style={ds.emptyWrap}>
                    <View style={ds.emptyIconBox}>
                      <ClipboardList size={36} color="#525C6B" />
                    </View>
...
const ds = StyleSheet.create({
  ...
  headerSub:  { fontSize: 10, fontWeight: '800', color: '#525C6B', textTransform: 'uppercase', letterSpacing: 0.8 },
  ...
  tabText:      { fontSize: 12, fontWeight: '600', color: '#525C6B' },
  ...
  heroMeta:       { fontSize: 12, color: '#fff' },
  ...
  heroMetaLabel:  { fontSize: 10, color: '#fff', textTransform: 'uppercase', letterSpacing: 0.4 },
  ...
  chartSub:       { fontSize: 11, color: '#525C6B', marginBottom: 12 },
  ...
  contentLabel: { fontSize: 10, fontWeight: '700', color: '#525C6B', textTransform: 'uppercase', textAlign: 'center' },

  // List header
  listHeaderRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 12 },
  listHeader:    { fontSize: 13, fontWeight: '700', color: '#525C6B' },
  studentSearchInput: {
    backgroundColor: '#fff',
    borderRadius: 12,
    borderWidth: 1,
  ...
  studentsPageBtnTextDisabled: { color: '#4E5D71' },
  studentsPageIndicator: { fontSize: 12, fontWeight: '700', color: '#4B5768' },
  ...
  studentEmail:   { fontSize: 11, color: '#525C6B', marginTop: 1 },
  ...
  progressLabel:  { fontSize: 10, fontWeight: '700', color: '#525C6B', textTransform: 'uppercase', letterSpacing: 0.4 },
  ...
  achievMore:  { fontSize: 11, color: '#525C6B', fontWeight: '700', alignSelf: 'center' },

  remarkBtn:          { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 10, borderRadius: 10, borderWidth: 1.5, borderColor: '#2D5DC9', paddingHorizontal: 12, paddingVertical: 8 },
  remarkBtnFilled:    { backgroundColor: '#2D5DC9', borderColor: '#2D5DC9' },
  remarkBtnText:      { fontSize: 12, fontWeight: '800', color: '#2D5DC9', flex: 1 },
  ...
  remarkScoreText:    { fontSize: 11, fontWeight: '800', color: '#2D5DC9' },
  ...
  secondaryActionBtnFilled: { backgroundColor: '#2D5DC9', borderColor: '#2D5DC9' },
  ...
  quizAttemptNumberText: { fontSize: 11, fontWeight: '800', color: '#4B5768' },
  ...
  detailFieldLabel: { fontSize: 10, fontWeight: '800', color: '#4B5768', textTransform: 'uppercase', letterSpacing: 0.45, marginBottom: 2 },
  ...
  dateTimeLabel: { fontSize: 10, fontWeight: '800', color: '#4B5768', minWidth: 34 },
  ...
  answerCardBannerTitle: { fontSize: 11, fontWeight: '800', color: '#4B5768' },
  ...
  mmBarLabel: { fontSize: 10, fontWeight: '700', color: '#525C6B', textTransform: 'uppercase', letterSpacing: 0.4 },
  mmBoardLabel: { fontSize: 10, fontWeight: '700', color: '#525C6B', textTransform: 'uppercase', letterSpacing: 0.4, marginTop: 4 },
  ...
  notSubmittedSub: { fontSize: 11, color: '#4B5768', textAlign: 'center', lineHeight: 16, maxWidth: 240 },
