...
export async function endLiveStoriesForClass(
  orgId: string,
  ...
  const params: any[] = [orgId, excludeId ?? '00000000-0000-0000-0000-000000000000'];
  if (classLevel != null) params.push(classLevel);
  await db.query(
    `UPDATE stories
    ...
    try {
      ...
      for (const row of due) {
        ...
        const updated = await db.query(
          `UPDATE stories SET status = 'live', published_at = NOW(), updated_at = NOW()
        ...
        } catch (err) { console.error('[story-service] scheduler publish failed', err); }
      }
