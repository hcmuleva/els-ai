...
export interface ExamFetchResult {
  examName: string;
  classLevel: string;
  subject: string;
  topic: string;
...
export async function fetchExamQuestions(params: {
  ...
  try {
    ...
    const questions: ExamQuestionItem[] = Array.isArray(response.questions)
          ...
          rawOptions.forEach((o: any, oIdx: number) => {
            let txt =
              typeof o === "string"
                ? o.trim()
                : String(
                    o?.text ?? o?.option ?? o?.choice ?? o?.value ?? o?.label ?? o?.answer ?? o?.content ?? ""
                  ).trim();
    ...
    const years = Array.from(
      new Set(questions.map((q) => q.examYear).filter(Boolean)),
    );
