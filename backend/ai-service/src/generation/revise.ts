 ...
 */
export async function runRevisionJob(job: GenerationJob, contentId: string, instruction: string) {
  ...
  try {
    for (const step of REVISE_STEPS) {
      ...
      if (step.key === "adding_content") {
        const revised = await generateJson<unknown>({
          system:
            "You are revising an existing piece of educational content. Reply with ONLY the " +
    ...
    const message = (err as Error).message ?? "Revision failed";
