...
import { fetchBestYoutubeVideo, type ValidatedYoutubeVideo } from "../../services/youtubeFetcher.js";
import { newContentId } from "../../utils/ids.js";
import type { TopicParams } from "../types.js";
...
const TopicOutput = z.object({
  ...
  const outlineList = (raw.outline || []).map((item: any, idx: number) => {
  ...
  const learningObjectives = Array.isArray(rawObjs)
    ? rawObjs.map((o: any) => String(o).trim())
...
export async function generateTopic(
  params: z.infer<typeof TopicParams>,
): Promise<GeneratorResult> {
  const output = await generateJson<z.infer<typeof TopicOutput>>({
    system:
      "You are a curriculum designer. Reply with ONLY JSON matching: " +
  ...
  let youtubeVideo: ValidatedYoutubeVideo | null = null;
  try {
