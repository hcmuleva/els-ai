...
import {
  fetchBestYoutubeVideo,
  fetchMultipleYoutubeVideos,
  type ValidatedYoutubeVideo,
} from "../../services/youtubeFetcher.js";
import { fetchExamQuestions, type ExamQuestionItem } from "../../services/examFetcher.js";
...
const ContentOutput = z
  ...
  .transform((raw: any) => {
    ...
    return {
      ...
      keyTakeaways: takeawaysList.map((t: any) => String(t).trim()),
...
export async function generateContent(
  ...
  const output = await generateJson<z.infer<typeof ContentOutput>>({
    system:
      "You are an expert curriculum writer. Reply with ONLY JSON matching: " +
  ...
  let youtubeVideos: ValidatedYoutubeVideo[] = [];
  try {
    if (desiredVideos > 1) {
      ...
      if (single) youtubeVideos.push(single);
    }
  } catch (err) {
  ...
  const contentSections: Array<{
    draftId: string;
    title: string;
    contentType: "youtube_url" | "text" | "quiz";
    mediaUrl: string;
    externalUrl: string;
    textContent: string;
    quizId: string | null;
