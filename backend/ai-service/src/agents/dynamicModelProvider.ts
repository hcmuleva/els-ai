 ...
 */
export async function fetchAvailableModels(rawBaseUrl: string, apiKey: string, timeoutMs = 4000): Promise<string[]> {
  ...
  try {
    ...
    if (Array.isArray(json?.data)) {
      list = json.data.map((m: any) => (typeof m === 'string' ? m : m?.id)).filter(Boolean);
    } else if (Array.isArray(json?.models)) {
      list = json.models.map((m: any) => (typeof m === 'string' ? m : m?.id || m?.name)).filter(Boolean);
    }
 ...
 */
export function createDynamicProvider(config: DynamicProviderConfig): AgentProvider {
  ...
  return {
    ...
    async *stream({ messages, model: requestModel, signal }: AgentStreamParams): AsyncGenerator<AgentStreamEvent> {
      if (!apiKey) {
