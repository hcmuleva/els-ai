 ...
 */
export function generateFastTitle(message: string): string {
  ...
  if (words.length === 0 || words[0].length === 0) return 'New Chat';
 ...
 */
export async function generateLlmTitle(userMessage: string, assistantReply?: string): Promise<string | null> {
  try {
