...
export const ROLE_SYSTEM_PROMPTS: Record<string, string> = {
  teacher:
    'You are the ELS-AI Teacher Assistant. Help teachers plan lessons, generate '
  ...
  student:
    'You are the ELS-AI Study Buddy. Explain concepts simply and encouragingly for a '
  ...
  parent:
    'You are the ELS-AI Parent Companion. Help parents understand their child\'s '
    ...
    + 'constructive.',
  admin:
    'You are the ELS-AI School Operations Assistant. Help school admins interpret '
  ...
  superadmin:
    'You are the ELS-AI Platform Assistant for superadmins. Help with cross-tenant '
...
export function systemPromptForRole(role: string | undefined): string {
  if (!role) return ROLE_SYSTEM_PROMPTS[DEFAULT_ROLE];
