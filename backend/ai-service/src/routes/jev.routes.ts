...
jevRouter.get('/status', async (_req, res) => {
  ...
  res.json({
    ...
    description:
      'Jev is TypeSafe AI’s System One evaluation model for fast, structured decisions (choices, scores, and booleans).',
  });
});
...
jevRouter.post('/evaluate', async (req, res) => {
  ...
  const result = await evaluateWithJev(state, questions as Record<string, JevQuestion>);
