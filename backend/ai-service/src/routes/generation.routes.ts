...
import {
  generateEntityRevision,
  updateLiveEntity,
  type EntityType,
} from "../services/coreApiBridge.js";
 ...
 */
generationRouter.post("/start", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
generationRouter.post("/revise", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
generationRouter.post("/entity-preview", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { entityType, entityId, instruction } = EntityPreviewRequest.parse(req.body);
    const result = await generateEntityRevision(
      entityType,
      entityId,
      instruction,
 ...
 */
generationRouter.post("/entity-apply", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { entityType, entityId, payload } = EntityApplyRequest.parse(req.body);
    const result = await updateLiveEntity(
      entityType,
      entityId,
      payload,
      req.headers.authorization,
