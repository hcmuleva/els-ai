...
chatRouter.post('/', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    if (studentContext) {
      const ctx = studentContext as any;
      ...
      const lowestQuizzesStr = metrics.lowestQuizzes?.length
        ? metrics.lowestQuizzes.map((q: any) => `${q.title} (${q.scorePct}%)`).join(', ')
        : 'None';
      const weakQuestionsStr = metrics.weakQuestions?.length
        ? metrics.weakQuestions.map((q: any) => `${q.title} (Missed ${q.missedCount}x)`).join('; ')
...
6. If the teacher asks to create a remedial quiz or lesson, then and only then propose a standard generation proposal.`;
          ...
          while (text.length > 0) {
            if (isInsideThink) {
              ...
              if (closeIdx !== -1) {
                ...
                if (thoughtChunk) writeSse(res, { thinking: thoughtChunk });
                isInsideThink = false;
    ...
    if (!canGenerateContent(role) && fullReply.includes('generation_proposal')) {
      fullReply = fullReply.replace(
        /```json\s*\{[^`]*?\"type\"\s*:\s*\"generation_proposal\"[^`]*?\}\s*```/gs,
    ...
    const looksLikeInlineContent = !hasProposalBlock && canGenerateContent(role) && (
