...
export const SUPPORTED_SUBJECTS: RequestedSubject[] = [
  'DoYouKnow',
  'TipsTricks',
  'HowToThink',
  'Story',
...
const SUBJECT_KEYWORDS: Record<RequestedSubject, string[]> = {
...
function detectLanguage(text: string): { language: 'Hindi' | 'English'; confidence: number } {
  const sample = normalizeText(text);
  if (!sample) return { language: 'English', confidence: 0.5 };

  const devanagariChars = (sample.match(/[\u0900-\u097F]/g) || []).length;
  const latinChars = (sample.match(/[A-Za-z]/g) || []).length;
  if (devanagariChars > latinChars * 0.3) return { language: 'Hindi', confidence: 0.86 };
...
function complexityFromScore(score: number): 'low' | 'medium' | 'high' {
  if (score < 0.34) return 'low';
  if (score < 0.67) return 'medium';
  return 'high';
}

function bandFromDifficultySignal(signal: number): string {
  const band = CLASS_BANDS.find((candidate) => signal >= candidate.min && signal <= candidate.max);
  return band ? band.label : 'Class 6';
}

function scoreSubjects(text: string): Record<RequestedSubject, number> {
  const normalized = text.toLowerCase();
  const scored = {} as Record<RequestedSubject, number>;
...
function calculateDifficultySignal(text: string, durationSeconds: number): number {
...
function failValidation(reason: string, confidence: number) {
  return {
    is_valid: false,
...
export function runContentPipeline(request: PipelineRequest) {
  ...
  for (const candidate of request.candidates || []) {
    ...
    const sourceType: SourceType = candidate.source_type || (isYoutubeUrl(url) ? 'youtube' : 'web');
    ...
    let validationResult:
      | {
          ...
          validated_duration_seconds: number;
          validation_confidence: number;
        }
      | ReturnType<typeof failValidation>;
    ...
    const [topSubject, topScore] = Object.entries(subjectScores).sort((a, b) => b[1] - a[1])[0] as [RequestedSubject, number];
