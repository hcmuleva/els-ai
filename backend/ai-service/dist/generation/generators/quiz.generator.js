...
export async function generateQuiz(params) {
    ...
    if (quizType === "mixed") {
        const output = await generateJson({
            system: "You are an expert curriculum writer. Reply with ONLY a JSON object: " +
        ...
        for (let idx = 0; idx < (output.questions || []).length; idx++) {
            ...
            let rawType = String(q.questionType || q.type || "").toLowerCase().trim();
            ...
            else if (rawType.includes("drag") || rawType.includes("match_box") || rawType.includes("drop") || (Array.isArray(q.pairs) && q.pairs.some((p) => p.item && p.target))) {
                rawType = "drag_drop_match";
            }
            else if (rawType.includes("memory") || (Array.isArray(q.pairs) && q.pairs.some((p) => p.emoji))) {
            ...
            else if (rawType.includes("true") || rawType.includes("false") || rawType === "tf" || (Array.isArray(q.options) && q.options.length === 2 && q.options.every((o) => /^(true|false)$/i.test(String(o).trim())))) {
                rawType = "true_false";
            }
            else {
    ...
    if (quizType === "jigsaw") {
        ...
        for (let i = 0; i < count; i++) {
            const puzzleTitle = count > 1 ? `${params.topic} Puzzle #${i + 1}` : `Jigsaw Puzzle: ${params.topic}`;
            const qData = {
                image: imageUrl,
