 ...
 */
export async function isYoutubeEmbeddable(videoId, timeoutMs = 4000) {
 ...
 */
function isLanguageDomain(str) {
    return /(hindi|हिंदी|english|sanskrit|marathi|gujarati|bengali|urdu|tamil|telugu|kannada|malayalam|punjabi|language|bhasha|भाषा|vocab|vocabulary|शब्दार्थ|grammar|व्याकरण|अनुच्छेद|गद्यांश|comprehension|passage|literature)/i.test(str || '');
}
function isMathDomain(str) {
    return /(math|maths|mathematics|ganit|गणित|अंकगणित|arithmetic|algebra|geometry|mental ability|reasoning|मानसिक योग्यता|tarkshakti|तर्कशक्ति|figure matching|fractions|decimals|mensuration)/i.test(str || '');
}
function isScienceDomain(str) {
    return /(science|physics|chemistry|biology|vigyan|विज्ञान|evs|environmental science)/i.test(str || '');
}
/**
 ...
 */
export function isSubjectConflicting(title, subject = '', topic = '') {
    ...
    if (isTargetLanguage && !isTargetMath) {
        if (/(math|maths|mathematics|ganit|अंकगणित|mental ability|reasoning|tarkshakti|मानसिक योग्यता|figure matching|arithmetic|geometry|algebra)/i.test(t)) {
            return true;
        }
    }
    // Math subject requested: strictly reject language/literature/grammar videos
    if (isTargetMath && !isTargetLanguage) {
        if (/(hindi|हिंदी|व्याकरण|bhasha|भाषा|गद्यांश|अनुच्छेद|english grammar|vocabulary|literature|comprehension)/i.test(t)) {
            return true;
        }
    }
    // Science subject requested: reject pure math or language grammar videos
    if (isTargetScience) {
        if (/(mental ability|reasoning|मानसिक योग्यता|figure matching|व्याकरण|english grammar)/i.test(t)) {
            return true;
        }
    }
    return false;
 ...
 */
function scoreVideoAffinity(title, subject = '', topic = '', gradeLevel = '') {
    const t = (title || '').toLowerCase();
 ...
 */
function buildSearchQueries(topic, subject = '', gradeLevel = '') {
...
export async function fetchBestYoutubeVideo(params) {
    ...
    for (const query of queries) {
        try {
            ...
            const getListByKeyword = ytSearch?.GetListByKeyword || ytSearch?.default?.GetListByKeyword;
            if (!getListByKeyword) {
            ...
            for (const candidate of scoredCandidates) {
                ...
                if (working) {
                    ...
                    const thumb = item.thumbnail?.thumbnails?.[item.thumbnail.thumbnails.length - 1]?.url ||
 ...
 */
export async function fetchMultipleYoutubeVideos(params, desiredCount = 2) {
    const { topic, subject = '', gradeLevel = '' } = params;
