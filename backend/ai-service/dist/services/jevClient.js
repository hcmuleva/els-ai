...
export function isJevConfigured() {
    return Boolean((process.env.OPENJEV_API_KEY && process.env.OPENJEV_API_KEY.trim().length > 0) ||
 ...
 */
function heuristicFallbackEvaluation(state, questions) {
    const result = {};
 ...
 */
export async function classifyMetricIntent(query) {
    const result = await jevClient.ask({ user_query: query }, {
        metric_domain: {
            type: 'choice',
    ...
    const domain = (result.data?.metric_domain?.choice || 'general');
    return {
        metricDomain: domain,
 ...
 */
export async function evaluateWithJev(state, questions, options) {
 ...
 */
export async function classifyWithJev(text, choices, instructions = 'Select the best matching category') {
    const res = await evaluateWithJev(text, {
        category: {
            type: 'choice',
            instructions,
            choices,
 ...
 */
export async function verifyWithJev(content, criteria) {
    const res = await evaluateWithJev(content, {
        satisfied: {
            type: 'boolean',
