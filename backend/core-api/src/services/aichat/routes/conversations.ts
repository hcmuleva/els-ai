...
aiConversationsRouter.post('/', requireAuth, async (req, res) => {
  ...
  const { rows } = await db.query(
    `INSERT INTO ai_conversations (user_id, organization_id, role, title)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [userId, organizationId, parsed.data.role, parsed.data.title || 'New conversation'],
  );
...
aiConversationsRouter.get('/:id/messages', requireAuth, async (req, res) => {
  const userId = getUserId(req);
  if (!userId) return res.status(400).json({ message: 'User not found in auth context' });
  const conversation = await loadOwnedConversation(req.params.id as string, userId);
...
aiConversationsRouter.post('/:id/messages', requireAuth, async (req, res) => {
  ...
  const { rows: existingRows } = await db.query(
    `SELECT COUNT(*)::int AS count FROM ai_messages WHERE conversation_id = $1`,
    [conversation.id],
  );
  ...
  const { rows } = await db.query(
    `INSERT INTO ai_messages (conversation_id, role, content)
     VALUES ($1, $2, $3)
     RETURNING *`,
    [conversation.id, parsed.data.role, parsed.data.content],
  );
  ...
  if (shouldRetitle) {
    await db.query(
      ...
      [parsed.data.titleHint!.slice(0, 200), conversation.id],
    );
  } else {
