...
function canManageFlags(req: AuthenticatedRequest): boolean {
...
featureFlagsRouter.get('/', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = getOrgId(req);
  if (!organizationId) return res.status(400).json({ message: 'Organization not found in auth context' });

  const { rows } = await db.query(
    `SELECT flag_key, enabled FROM feature_flags WHERE organization_id = $1`,
    [organizationId],
  );
  const overrides = new Map<string, boolean>(rows.map((r) => [r.flag_key as string, r.enabled as boolean]));
  return res.json({
...
featureFlagsRouter.patch('/:key', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = getOrgId(req);
  if (!organizationId) return res.status(400).json({ message: 'Organization not found in auth context' });
  if (!canManageFlags(req)) return res.status(403).json({ message: 'Admin access required' });

  const key = req.params.key as string;
  if (!FEATURE_FLAG_KEYS.has(key)) return res.status(404).json({ message: 'Unknown feature flag' });
  ...
  await db.query(
    `INSERT INTO feature_flags (organization_id, flag_key, enabled, updated_by, updated_at)
     VALUES ($1, $2, $3, $4, NOW())
     ON CONFLICT (organization_id, flag_key)
     DO UPDATE SET enabled = $3, updated_by = $4, updated_at = NOW()`,
    [organizationId, key, parsed.data.enabled, userId],
  );
