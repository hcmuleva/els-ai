...
import {
  buildCounselingReport,
  extractOpenResponses,
  type CounselingAnswers,
  type StudentSnapshot,
} from '../services/counseling-scoring.js';
...
async function canAccessStudent(req: AuthenticatedRequest, studentId: string, organizationId: string | null) {
  const user = req.user;
  if (!organizationId || !user?.userId) return false;
  if (user.role === 'admin' || user.role === 'superadmin' || user.userId === studentId) return true;
  return isParentOfStudent(user.userId, studentId, organizationId);
}

// Parents (or admins) own the counseling flow.
async function canRunCounseling(req: AuthenticatedRequest, studentId: string, organizationId: string | null) {
  const user = req.user;
  if (!organizationId || !user?.userId) return false;
...
async function loadAuthorizedSession(req: AuthenticatedRequest, sessionId: string, organizationId: string) {
  const result = await db.query(
    `SELECT id, parent_user_id, student_user_id, organization_id, status,
  ...
  if (!row) return null;
  if (!(await canAccessStudent(req, row.student_user_id as string, organizationId))) return null;
  return row;
}

async function loadAnswers(sessionId: string): Promise<CounselingAnswers> {
  const result = await db.query(
    `SELECT question_key, value_json FROM counseling_responses WHERE session_id = $1::uuid`,
    [sessionId],
  );
  const answers: CounselingAnswers = {};
  for (const row of result.rows) answers[row.question_key as string] = row.value_json;
  return answers;
}
...
counselingRouter.post('/sessions', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = orgId(req);
  ...
  try {
    const result = await db.query(
      `INSERT INTO counseling_sessions (parent_user_id, student_user_id, organization_id, student_snapshot)
       VALUES ($1, $2, $3::uuid, $4::jsonb)
       RETURNING id, status, student_snapshot, started_at, created_at`,
      [req.user?.userId, parsed.data.studentId, organizationId, JSON.stringify(parsed.data.snapshot ?? {})],
    );
    const row = result.rows[0];
...
counselingRouter.get('/sessions/:id', requireAuth, async (req: AuthenticatedRequest, res) => {
  const sessionId = getSingleParam(req.params.id);
  const organizationId = orgId(req);
  if (!sessionId || !organizationId) return res.status(400).json({ message: 'Invalid request' });
  ...
  return res.json({
    ...
    studentId: session.student_user_id as string,
    status: session.status as string,
    snapshot: session.student_snapshot,
    durationSec: Number(session.duration_sec || 0),
    startedAt: session.started_at as string,
    submittedAt: (session.submitted_at as string | null) || null,
    answers,
  });
});

// PATCH /counseling/sessions/:id/responses — autosave answers (upsert).
counselingRouter.patch('/sessions/:id/responses', requireAuth, async (req: AuthenticatedRequest, res) => {
  const sessionId = getSingleParam(req.params.id);
  const organizationId = orgId(req);
  ...
  const session = await loadAuthorizedSession(req, sessionId, organizationId);
  if (!session) return res.status(404).json({ message: 'Session not found' });
  if (session.status === 'reported') return res.status(409).json({ message: 'Session already reported' });

  try {
    for (const item of parsed.data.responses) {
      await db.query(
        `INSERT INTO counseling_responses (session_id, organization_id, section, question_key, value_json)
         VALUES ($1::uuid, $2::uuid, $3, $4, $5::jsonb)
         ON CONFLICT (session_id, question_key) DO UPDATE
           SET value_json = EXCLUDED.value_json, section = EXCLUDED.section, updated_at = NOW()`,
        [sessionId, organizationId, item.section, item.questionKey, JSON.stringify(item.value ?? null)],
      );
    }
...
counselingRouter.post('/sessions/:id/submit', requireAuth, async (req: AuthenticatedRequest, res) => {
  const sessionId = getSingleParam(req.params.id);
  const organizationId = orgId(req);
  ...
  const session = await loadAuthorizedSession(req, sessionId, organizationId);
  if (!session) return res.status(404).json({ message: 'Session not found' });
  if (!(await canRunCounseling(req, session.student_user_id as string, organizationId))) {
    return res.status(403).json({ message: 'Forbidden' });
  }
    ...
    await db.query(
      `UPDATE counseling_sessions
          SET status = CASE WHEN status = 'reported' THEN status ELSE 'submitted' END,
              ...
              updated_at = NOW()
        WHERE id = $1::uuid`,
      [sessionId, parsed.data.durationSec ?? 0],
    );
...
counselingRouter.post('/sessions/:id/report', requireAuth, async (req: AuthenticatedRequest, res) => {
  const sessionId = getSingleParam(req.params.id);
  const organizationId = orgId(req);
  if (!sessionId || !organizationId) return res.status(400).json({ message: 'Invalid request' });
  ...
  try {
    const answers = await loadAnswers(sessionId);
    const snapshot = (session.student_snapshot || {}) as StudentSnapshot;
    ...
    const inserted = await db.query(
      `INSERT INTO counseling_reports
    ...
    await db.query(
      `UPDATE counseling_sessions SET status = 'reported', updated_at = NOW() WHERE id = $1::uuid`,
      [sessionId],
    );
...
counselingRouter.get('/sessions/:id/report', requireAuth, async (req: AuthenticatedRequest, res) => {
  const sessionId = getSingleParam(req.params.id);
  const organizationId = orgId(req);
  if (!sessionId || !organizationId) return res.status(400).json({ message: 'Invalid request' });
  ...
  try {
    const result = await db.query(
      `SELECT id, overall_score, level, growth_potential, study_pattern_type, report_json, created_at
...
counselingRouter.get('/students/:studentId/sessions', requireAuth, async (req: AuthenticatedRequest, res) => {
  const studentId = getSingleParam(req.params.studentId);
  const organizationId = orgId(req);
  if (!studentId || !organizationId) return res.status(400).json({ message: 'Invalid request' });
  if (!(await canAccessStudent(req, studentId, organizationId))) return res.status(403).json({ message: 'Forbidden' });

  try {
    const result = await db.query(
      `SELECT s.id, s.status, s.started_at, s.submitted_at, s.duration_sec,
    ...
    return res.json({
      sessions: result.rows.map((row) => ({
        ...
        reportCreatedAt: (row.report_created_at as string | null) || null,
      })),
    });
  } catch (error) {
    console.error(error);
...
counselingRouter.delete('/students/:studentId/sessions/pending', requireAuth, async (req: AuthenticatedRequest, res) => {
  const studentId = getSingleParam(req.params.studentId);
  const organizationId = orgId(req);
  if (!studentId || !organizationId) return res.status(400).json({ message: 'Invalid request' });
  if (!(await canRunCounseling(req, studentId, organizationId))) return res.status(403).json({ message: 'Forbidden' });
