...
import { z } from 'zod';
import { requireAuth, canManage, type AuthenticatedRequest } from '../middleware/auth.js';
...
async function getOccupiedDays(orgId: string, classLevel: string | null): Promise<string[]> {
  ...
  const result = await db.query<{ day: string }>(
    `SELECT DISTINCT to_char(COALESCE(published_at, scheduled_at), 'YYYY-MM-DD') AS day
       FROM stories
...
function getClassLevel(req: AuthenticatedRequest): string | null {
  return (req as any).user?.classLevel || null;
}
...
storiesRouter.get('/', requireAuth, async (req: any, res) => {
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
...
storiesRouter.get('/scheduled-days', requireAuth, async (req: any, res) => {
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
...
storiesRouter.get('/home/feed', requireAuth, async (req: any, res) => {
  const orgId = getOrgId(req);
  const userId = getUserId(req);
  if (!orgId || !userId) return res.status(400).json({ message: 'Auth missing' });
...
storiesRouter.get('/:id', requireAuth, async (req: any, res) => {
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
...
storiesRouter.post('/', requireAuth, async (req: any, res) => {
...
storiesRouter.patch('/:id', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
...
storiesRouter.patch('/:id/schedule', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req); const userId = getUserId(req);
  if (!orgId || !userId) return res.status(400).json({ message: 'Auth missing' });
  const parsed = scheduleStorySchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: 'Invalid schedule', errors: parsed.error.issues });
  const ms = new Date(parsed.data.scheduledAt).getTime();
  if (Number.isNaN(ms) || ms <= Date.now()) return res.status(400).json({ message: 'Scheduled time must be in the future' });
  ...
  if (conflict.length > 0) {
    const selfCheck = await db.query(
      `SELECT id FROM stories WHERE organization_id = $1::uuid AND status IN ('live','scheduled')
         AND to_char(COALESCE(published_at, scheduled_at), 'YYYY-MM-DD') = $2
         AND id <> $3::uuid AND (class_level = $4 OR ($4::text IS NULL AND class_level IS NULL))`,
      [orgId, targetDay, req.params.id, story.class_level],
    );
  ...
  } catch (err) { console.error('[story-service] publish failed', err); }
  return res.json({ story: toStoryDto(row) });
});

storiesRouter.patch('/:id/publish', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req); const userId = getUserId(req);
  if (!orgId || !userId) return res.status(400).json({ message: 'Auth missing' });
  ...
  if (!storyToPublish) return res.status(404).json({ message: 'Story not found' });
  ...
  const selfCheck = await db.query(
    `SELECT id FROM stories WHERE organization_id = $1::uuid AND status IN ('live','scheduled')
       AND to_char(COALESCE(published_at, scheduled_at), 'YYYY-MM-DD') = $2
       AND id <> $3::uuid AND (class_level = $4 OR ($4::text IS NULL AND class_level IS NULL))`,
    [orgId, todayDay, req.params.id, storyToPublish.class_level],
  );
  if ((selfCheck.rowCount ?? 0) > 0) {
  ...
  const result = await db.query(
    `UPDATE stories SET status = 'live', scheduled_at = NULL, published_at = NOW(), updated_at = NOW()
       WHERE id = $1::uuid AND organization_id = $2::uuid RETURNING *`,
    [req.params.id, orgId],
  );
  if (!result.rowCount) return res.status(404).json({ message: 'Story not found' });
...
storiesRouter.patch('/:id/end', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req); const userId = getUserId(req);
  if (!orgId || !userId) return res.status(400).json({ message: 'Auth missing' });
...
storiesRouter.delete('/:id', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
  const ok = await StoryStore.delete(orgId, req.params.id);
  if (!ok) return res.status(404).json({ message: 'Story not found' });
  return res.json({ deleted: true });
});

storiesRouter.get('/:id/sections', requireAuth, async (req: any, res) => {
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
  const story = await StoryStore.getById(orgId, req.params.id);
  if (!story) return res.status(404).json({ message: 'Story not found' });
...
storiesRouter.post('/:id/sections', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
  const story = await StoryStore.getById(orgId, req.params.id);
...
storiesRouter.patch('/:id/sections/:sectionId', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
  const story = await StoryStore.getById(orgId, req.params.id);
...
storiesRouter.delete('/:id/sections/:sectionId', requireAuth, async (req: any, res) => {
  if (!canManage(req)) return res.status(403).json({ message: 'Forbidden' });
  const orgId = getOrgId(req);
  if (!orgId) return res.status(400).json({ message: 'Org missing' });
  const story = await StoryStore.getById(orgId, req.params.id);
...
storiesRouter.get('/:id/progress', requireAuth, async (req: any, res) => {
  const userId = getUserId(req);
  if (!userId) return res.status(400).json({ message: 'User missing' });
  const progress = await StoryStore.getProgress(userId, req.params.id);
  return res.json({ progress });
});

storiesRouter.post('/:id/progress', requireAuth, async (req: any, res) => {
  const userId = getUserId(req);
  if (!userId) return res.status(400).json({ message: 'User missing' });
  const parsed = progressSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: 'Invalid payload', errors: parsed.error.issues });
  const existing = await StoryStore.getProgress(userId, req.params.id);
  const completedSet = new Set<string>(existing?.completed_section_ids || []);
  if (parsed.data.sectionId) completedSet.add(parsed.data.sectionId);
