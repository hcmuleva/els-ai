...
quizzesRouter.post('/', requireAuth, async (req: any, res) => {
  ...
  try {
    ...
    if (!resolvedSubjectId && subject) {
      ...
      let lookup = await db.query(
        `SELECT s.id FROM subjects s
         ...
         LIMIT 1`,
        [targetClass, cleanSubj, orgId, isGlobal]
      );
    ...
    const result = await db.query(
      ...
      [orgId, title, description || null, targetClass || null, resolvedSubjectId, normalizedQuizType, difficultyLevel || null, backgroundMusicUrl || null, theme, isPublished, isAiGenerated, userId, isGlobal],
    );
    ...
    if (Array.isArray(questions) && questions.length > 0) {
      for (let i = 0; i < questions.length; i++) {
        ...
        const formattedOptions = rawOpts.map((opt: any, optIdx: number) => {
        ...
        const isBoolQuestion = formattedOptions.length === 2 &&
          formattedOptions.every((o: any) => /^(true|false|yes|no)$/i.test(String(o.text || '').trim()));
        ...
        if (!isBoolQuestion && formattedOptions.length > 1) {
          ...
          formattedOptions.forEach((opt: any, idx: number) => {
            opt.id = `opt-${idx + 1}`;
          });
        }

        const correctIdx = formattedOptions.findIndex((o: any) => o.is_correct);
        ...
        if (!qData) {
          if (qType === 'fill_blank') {
            qData = {
              ...
              options: rawOpts.length > 0 ? rawOpts : formattedOptions.map((o: any) => o.text),
        ...
        const insResult = await db.query(
          `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, explanation, question_audio, time_limit_seconds, points, sort_order, question_data)
           ...
           RETURNING *`,
          [createdQuizId, qType, qTitle, q.questionInstruction || null, qExplanation, q.questionAudio || null, q.timeLimitSeconds || 30, q.points || 10, i + 1, qData]
        );
        if (insResult.rows[0]) {
      ...
      await db.query(
        `UPDATE quizzes SET total_questions = $1, updated_at = NOW() WHERE id = $2`,
        [insertedQuestions.length, createdQuizId]
      );
