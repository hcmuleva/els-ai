...
export const contentRepository = {
  async findById(contentId: string, organizationId: string) {
    const result = await db.query(
      `SELECT id, title, content_type, content_url, content_text, organization_id,
  ...
  async countForOrganization(organizationId: string): Promise<number> {
    const result = await db.query<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM learning_contents
