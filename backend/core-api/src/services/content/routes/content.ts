...
contentRouter.get('/_meta', requireAuth, async (_req: AuthenticatedRequest, res) => {
  const totals = await db.query<{ count: string }>(`SELECT COUNT(*)::text AS count FROM learning_contents`);
  res.json({
...
contentRouter.get('/subjects', requireAuth, async (req: any, res) => {
  ...
  try {
    const result = await db.query(
      `SELECT s.id, s.title, s.class_level, s.class_id, cl.code AS resolved_class_id, s.cover_image, s.icon_image, s.icon_bg_color
       FROM subjects s
       LEFT JOIN class_levels cl ON cl.code = s.class_level
       WHERE ${whereClauses.join(' AND ')}
       ORDER BY cl.display_order ASC NULLS LAST, s.class_level ASC, s.title ASC`,
      params,
    );
...
contentRouter.post('/items', requireAuth, async (req: any, res) => {
...
contentRouter.get('/items', requireAuth, async (req: any, res) => {
  ...
  const params: unknown[] = [orgId];
  const whereClauses: string[] = ['(lc.organization_id = $1::uuid OR lc.is_global = true)'];
...
contentRouter.get('/items/:contentId', requireAuth, async (req: any, res) => {
  ...
  try {
    const contentResult = await db.query(
      `SELECT lc.id, lc.class_level, s.title AS subject, lc.title, lc.content_type, lc.media_url, lc.external_url, lc.text_content, lc.created_by, lc.is_global, lc.created_at, lc.updated_at
       FROM learning_contents lc
       LEFT JOIN subjects s ON s.id = lc.subject_id
       WHERE lc.id = $1
...
contentRouter.put('/items/:contentId', requireAuth, async (req: any, res) => {
  const contentId = req.params.contentId as string;
...
contentRouter.delete('/items/:contentId', requireAuth, async (req: any, res) => {
  const contentId = req.params.contentId as string;
  if (!contentId) {
    return res.status(400).json({ message: 'Invalid content id' });
  }
