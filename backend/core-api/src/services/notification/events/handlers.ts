...
async function pushToUserChannel(orgId: string, userId: string, name: string, data: Record<string, unknown>) {
...
export async function registerNotificationHandlers() {
  ...
  await eventBus.subscribe<RemarkCreatedPayload>('notification.requested', async (event) => {
    ...
    if (event.payload && (event.payload as any).kind === 'feedback_thread_created') {
      const orgId = event.organizationId;
      if (!orgId) return;
    ...
    if (event.payload && (event.payload as any).kind === 'feedback_message') {
      const orgId = event.organizationId;
      if (!orgId) return;
      const data = event.payload as any;
      const studentId = data.studentUserId as string;
