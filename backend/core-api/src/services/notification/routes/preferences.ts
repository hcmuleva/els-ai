...
preferencesRouter.get('/', requireAuth, async (req, res) => {
  ...
  const result = await db.query(
    `SELECT user_id, organization_id, enabled_types, auto_delete_days, updated_at
...
preferencesRouter.put('/', requireAuth, async (req, res) => {
  ...
  await db.query(
    `INSERT INTO notification_preferences (user_id, organization_id, enabled_types, auto_delete_days, updated_at)
     ...
     ON CONFLICT (user_id) DO UPDATE
       SET enabled_types = EXCLUDED.enabled_types,
           ...
           updated_at = NOW()`,
    [userId, orgId, JSON.stringify(enabledTypes), autoDeleteDays],
  );
