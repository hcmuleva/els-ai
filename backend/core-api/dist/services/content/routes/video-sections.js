...
async function loadSection(sectionId, orgId) {
    const result = await db.query(`SELECT * FROM video_sections WHERE id = $1 AND organization_id = $2::uuid`, [sectionId, orgId]);
    return (result.rowCount ?? 0) > 0 ? result.rows[0] : null;
}
// ── Create section ────────────────────────────────────────────────────────
// POST /content/:contentId/video-sections
videoContentRouter.post('/:contentId/video-sections', async (req, res) => {
    ...
    const contentResult = await db.query(`SELECT id, video_duration FROM learning_contents WHERE id = $1 AND (organization_id = $2::uuid OR is_global = true)`, [contentId, orgId]);
    if ((contentResult.rowCount ?? 0) === 0) {
        return res.status(404).json({ message: 'Content not found' });
    }
    const videoDuration = contentResult.rows[0].video_duration;
    ...
    try {
        const orderResult = await db.query(`SELECT COALESCE(MAX(section_order), 0) + 1 AS next_order
         FROM video_sections WHERE content_id = $1 AND content_section_order = $2`, [contentId, contentSectionOrder]);
        const nextOrder = Number(orderResult.rows[0].next_order || 1);
        const inserted = await db.query(`INSERT INTO video_sections
...
videoContentRouter.get('/:contentId/video-sections', async (req, res) => {
    const orgId = getOrganizationId(req);
    if (!orgId)
        return res.status(400).json({ message: 'Organization not found in auth context' });
    const { contentId } = req.params;
    ...
    const sectionOrder = rawSectionOrder !== undefined && rawSectionOrder !== '' ? Number(rawSectionOrder) : null;
    const params = [contentId, orgId];
...
videoContentRouter.get('/:contentId/video-progress', async (req, res) => {
    ...
    if (!orgId)
        return res.status(400).json({ message: 'Organization not found in auth context' });
    const { contentId } = req.params;
    const sectionsResult = await db.query(`SELECT id, title, quiz_id, status, start_time, end_time, section_order
     FROM video_sections WHERE content_id = $1 AND organization_id = $2::uuid
     ORDER BY start_time ASC`, [contentId, orgId]);
    ...
    const progressResult = await db.query(`SELECT svp.student_id, svp.section_id, svp.video_watch_status, svp.quiz_status,
            svp.quiz_score,
     ...
     WHERE svp.content_id = $1 AND svp.organization_id = $2::uuid`, [contentId, orgId]);
...
videoSectionsRouter.patch('/:sectionId', async (req, res) => {
    if (!canManageTeacherContent(req)) {
        return res.status(403).json({ message: 'Forbidden: teacher/admin role required' });
    }
    const orgId = getOrganizationId(req);
    ...
    if (data.learningObjective !== undefined)
        set('learning_objective', data.learningObjective);
    if (data.ageGroup !== undefined)
        set('age_group', data.ageGroup);
    ...
    try {
        const result = await db.query(`UPDATE video_sections SET ${fields.join(', ')}
...
videoSectionsRouter.delete('/:sectionId', async (req, res) => {
    if (!canManageTeacherContent(req)) {
        return res.status(403).json({ message: 'Forbidden: teacher/admin role required' });
    }
    const orgId = getOrganizationId(req);
    if (!orgId)
        return res.status(400).json({ message: 'Organization not found in auth context' });
    const result = await db.query(`DELETE FROM video_sections WHERE id = $1 AND organization_id = $2::uuid RETURNING id`, [req.params.sectionId, orgId]);
...
videoSectionsRouter.post('/:sectionId/quiz', async (req, res) => {
    if (!canManageTeacherContent(req)) {
        return res.status(403).json({ message: 'Forbidden: teacher/admin role required' });
    }
    const orgId = getOrganizationId(req);
    ...
    if (!existing)
        return res.status(404).json({ message: 'Section not found' });
    const quizCheck = await db.query(`SELECT id FROM quizzes WHERE id = $1 AND (organization_id = $2::uuid OR is_global = true)`, [parsed.data.quizId, orgId]);
    if ((quizCheck.rowCount ?? 0) === 0)
        return res.status(404).json({ message: 'Quiz not found' });
    try {
        const result = await db.query(`UPDATE video_sections
       SET quiz_id = $1, updated_at = NOW()
       WHERE id = $2 AND organization_id = $3::uuid
       RETURNING *`, [parsed.data.quizId, req.params.sectionId, orgId]);
        return res.json(mapSection(result.rows[0]));
    }
    catch (error) {
        console.error(error);
...
videoSectionsRouter.delete('/:sectionId/quiz', async (req, res) => {
    if (!canManageTeacherContent(req)) {
        return res.status(403).json({ message: 'Forbidden: teacher/admin role required' });
    }
    const orgId = getOrganizationId(req);
    ...
    const result = await db.query(`UPDATE video_sections
     ...
     RETURNING *`, [req.params.sectionId, orgId]);
    return res.json(mapSection(result.rows[0]));
});
// ── Get quiz for active section (dynamic quiz panel) ─────────────────────────
// GET /video-sections/:sectionId/quiz
videoSectionsRouter.get('/:sectionId/quiz', async (req, res) => {
    const orgId = getOrganizationId(req);
    if (!orgId)
        return res.status(400).json({ message: 'Organization not found in auth context' });
    ...
    const quizResult = await db.query(`SELECT id, title, description, difficulty_level FROM quizzes WHERE id = $1`, [section.quiz_id]);
    if ((quizResult.rowCount ?? 0) === 0)
        return res.json({ quiz: null });
    const questionsResult = await db.query(`SELECT id, question_type, question_title, question_instruction, points, sort_order, question_data
     FROM quiz_questions WHERE quiz_id = $1 ORDER BY sort_order ASC, created_at ASC`, [section.quiz_id]);
    return res.json({
        quiz: {
...
videoSectionsRouter.post('/:sectionId/quiz/submit', async (req, res) => {
    const orgId = getOrganizationId(req);
    const studentId = getUserId(req);
    if (!orgId || !studentId)
        return res.status(400).json({ message: 'Auth context incomplete' });
    const parsed = submitQuizSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ message: 'score and totalPoints are required' });
    const section = await loadSection(req.params.sectionId, orgId);
    if (!section)
        return res.status(404).json({ message: 'Section not found' });
    const pct = parsed.data.totalPoints > 0
        ...
        : 0;
    const result = await db.query(`INSERT INTO student_video_progress
     ...
     RETURNING *`, [studentId, section.content_id, section.id, orgId, pct]);
...
videoSectionsRouter.post('/:sectionId/progress', async (req, res) => {
    const orgId = getOrganizationId(req);
    const studentId = getUserId(req);
    if (!orgId || !studentId)
        return res.status(400).json({ message: 'Auth context incomplete' });
    ...
    if (!parsed.success)
        return res.status(400).json({ message: 'Invalid progress payload', errors: parsed.error.issues });
    const section = await loadSection(req.params.sectionId, orgId);
    if (!section)
        return res.status(404).json({ message: 'Section not found' });
    ...
    const result = await db.query(`INSERT INTO student_video_progress
     ...
     ON CONFLICT (student_id, section_id) DO UPDATE
       SET video_watch_status = EXCLUDED.video_watch_status,
           ...
           updated_at = NOW()
     RETURNING *`, [studentId, section.content_id, section.id, orgId, watchStatus, watchedSeconds]);
    const row = result.rows[0];
    return res.json({
        status: 'saved',
        videoWatchStatus: row.video_watch_status,
...
videoSectionsRouter.get('/content/:contentId/my-progress', async (req, res) => {
    const orgId = getOrganizationId(req);
    const studentId = getUserId(req);
    if (!orgId || !studentId)
        return res.status(400).json({ message: 'Auth context incomplete' });
    const result = await db.query(`SELECT section_id, video_watch_status, quiz_status, watched_seconds, quiz_score, completed_at
     FROM student_video_progress
     WHERE content_id = $1 AND student_id = $2::uuid AND organization_id = $3::uuid`, [req.params.contentId, studentId, orgId]);
...
//# sourceMappingURL=video-sections.js.map