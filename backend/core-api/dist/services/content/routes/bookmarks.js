...
bookmarksRouter.get('/', requireAuth, async (req, res) => {
    ...
    const params = [orgId, teacherId];
    const whereClauses = ['b.organization_id = $1::uuid', 'b.teacher_user_id = $2::uuid'];
    if (search) {
        params.push(`%${search}%`);
    ...
    if (type) {
        params.push(type);
        whereClauses.push(`EXISTS (SELECT 1 FROM teacher_bookmark_items ti WHERE ti.bookmark_id = b.id AND ti.item_type = $${params.length})`);
    }
    if (subject) {
        params.push(subject);
...
bookmarksRouter.get('/:id', requireAuth, async (req, res) => {
    ...
    try {
        const bookmarkResult = await db.query(`SELECT id, name, description, class_level, created_at, updated_at
       FROM teacher_bookmarks
       WHERE id = $1::uuid AND organization_id = $2::uuid AND teacher_user_id = $3::uuid`, [bookmarkId, orgId, teacherId]);
       ...
       ORDER BY i.sort_order ASC, i.created_at ASC`, [bookmarkId]);
        return res.json({
            bookmark: {
                id: bookmark.id,
                name: bookmark.name,
                description: bookmark.description || undefined,
                classLevel: bookmark.class_level || undefined,
                ...
                items: itemsResult.rows.map((row) => ({
                    ...
                    classLevel: row.class_level || undefined,
                    subject: row.subject_title || undefined,
                    topicTitle: row.topic_title || undefined,
                    title: (row.item_type === 'content' ? row.content_title : row.quiz_title),
                    contentType: row.content_type || undefined,
                    quizType: row.quiz_type || undefined,
...
async function replaceBookmarkItems(client, bookmarkId, items) {
    await client.query('DELETE FROM teacher_bookmark_items WHERE bookmark_id = $1::uuid', [bookmarkId]);
    let sortOrder = 0;
    for (const item of items) {
        await client.query(`INSERT INTO teacher_bookmark_items
...
bookmarksRouter.post('/', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const inserted = await client.query(`INSERT INTO teacher_bookmarks (organization_id, teacher_user_id, name, description, class_level)
       VALUES ($1::uuid, $2::uuid, $3, $4, $5)
       RETURNING id`, [orgId, teacherId, name, description || null, classLevel || null]);
...
bookmarksRouter.put('/:id', requireAuth, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const owned = await client.query(`SELECT 1 FROM teacher_bookmarks
       WHERE id = $1::uuid AND organization_id = $2::uuid AND teacher_user_id = $3::uuid`, [bookmarkId, orgId, teacherId]);
        if (owned.rowCount === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ message: 'Bookmark not found' });
        }
        await client.query(`UPDATE teacher_bookmarks
       SET name = $1, description = $2, class_level = $3, updated_at = NOW()
       WHERE id = $4::uuid`, [name, description || null, classLevel || null, bookmarkId]);
        await replaceBookmarkItems(client, bookmarkId, items);
        await client.query('COMMIT');
...
bookmarksRouter.delete('/:id', requireAuth, async (req, res) => {
    ...
    try {
        const deleted = await db.query(`DELETE FROM teacher_bookmarks
       WHERE id = $1::uuid AND organization_id = $2::uuid AND teacher_user_id = $3::uuid
       RETURNING id`, [bookmarkId, orgId, teacherId]);
