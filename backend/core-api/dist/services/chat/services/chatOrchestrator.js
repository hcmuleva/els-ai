...
async function callJevSurveyTurn(params) {
    ...
    if (isWeak)
        category = 'weakness_academic';
    else if (isStrong)
        category = 'strength_academic';
    else if (isBehav)
        category = 'behavior';
 ...
 */
async function classifyDeepDiveIntent(query) {
    const q = query.toLowerCase();
    if (q.includes('read') || q.includes('book') || q.includes('phonic') || q.includes('word'))
        return 'reading';
    if (q.includes('math') || q.includes('count') || q.includes('number') || q.includes('add'))
        return 'math';
    if (q.includes('quiz') || q.includes('test') || q.includes('score') || q.includes('grade'))
        return 'quizzes';
    if (q.includes('attend') || q.includes('streak') || q.includes('active') || q.includes('time'))
        return 'attendance';
    if (q.includes('behavior') || q.includes('remark') || q.includes('teacher') || q.includes('focus'))
        return 'behavior';
    return 'general';
}
export const ChatOrchestrator = {
    async createSession(params) {
        ...
        if (studentId) {
            const studentRes = await db.query(`SELECT name FROM users WHERE id = $1::uuid LIMIT 1`, [studentId]);
        ...
        const sessionRes = await db.query(`INSERT INTO chat_sessions (
      ...
      RETURNING *`, [
        ...
        const msgRes = await db.query(`INSERT INTO chat_messages (
    ...
    async handleUserMessage(params) {
        // 1. Fetch Session
        const sessionRes = await db.query(`SELECT * FROM chat_sessions WHERE id = $1::uuid AND organization_id = $2::uuid LIMIT 1`, [params.sessionId, params.organizationId]);
        const session = sessionRes.rows[0];
        if (!session) {
            throw new Error('Chat session not found');
        }
        // 2. Persist User Message
        const userMsgRes = await db.query(`INSERT INTO chat_messages (session_id, organization_id, sender, content)
       VALUES ($1::uuid, $2::uuid, 'user', $3)
       RETURNING *`, [session.id, params.organizationId, params.content]);
        ...
        if (session.chat_type === 'survey') {
            ...
            if (needsClarification) {
                ...
                await db.query(`INSERT INTO chat_survey_entries (
            ...
            specific_enough, concern_level, confidence, review_status, raw_source_text, extracted_summary
                ...
                if (nextIndex < SURVEY_TOPICS.length) {
                    ...
                    await db.query(`UPDATE chat_sessions SET status = 'completed' WHERE id = $1::uuid`, [session.id]);
                }
            }
        }
        else {
            ...
            const quizRes = await db.query(`SELECT COUNT(*)::text as count, COALESCE(AVG(score), 0)::numeric(5,1)::text as avg_score
         FROM classroom_assignment_submissions
         WHERE student_id = $1::uuid`, [studentId]);
            const quizzesTaken = quizRes.rows[0]?.count || '0';
            const avgScore = quizRes.rows[0]?.avg_score || '0';
            const remarksRes = await db.query(`SELECT remark, category FROM classroom_student_remarks
         WHERE student_id = $1::uuid
         ORDER BY created_at DESC LIMIT 2`, [studentId]);
        ...
        const assistantMsgRes = await db.query(`INSERT INTO chat_messages (session_id, organization_id, sender, content, metadata)
       VALUES ($1::uuid, $2::uuid, 'assistant', $3, $4::jsonb)
       RETURNING *`, [session.id, params.organizationId, assistantContent, JSON.stringify(metadata)]);
        const assistantMessage = assistantMsgRes.rows[0];
        // Update Context Slots on Session
        const updatedSessionRes = await db.query(`UPDATE chat_sessions
       SET context_slots = $1::jsonb, updated_at = NOW()
       WHERE id = $2::uuid
       RETURNING *`, [JSON.stringify(slots), session.id]);
    ...
    async getReviewQueue(params) {
        ...
        const res = await db.query(`SELECT e.*, u.name as student_name, s.chat_type
       ...
       ORDER BY e.created_at DESC`, [params.organizationId, status]);
        return res.rows;
    },
    async updateReviewEntry(params) {
        const res = await db.query(`UPDATE chat_survey_entries
       SET review_status = $1,
       ...
       RETURNING *`, [params.action, params.teacherNotes || null, params.reviewerId, params.entryId, params.organizationId]);
        return res.rows[0];
    },
};
