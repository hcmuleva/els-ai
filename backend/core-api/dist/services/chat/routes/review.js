...
chatReviewRouter.get('/', requireAuth, async (req, res) => {
    const user = req.user;
...
chatReviewRouter.get('/stats', requireAuth, async (req, res) => {
    const user = req.user;
    if (!user?.organizationId)
        return res.status(401).json({ message: 'Unauthorized' });
    try {
        const statsRes = await db.query(`SELECT review_status, COUNT(*)::text as count
       FROM chat_survey_entries
       WHERE organization_id = $1::uuid
       GROUP BY review_status`, [user.organizationId]);
        const counts = {
            pending_review: 0,
            auto_accepted: 0,
            approved: 0,
...
chatReviewRouter.patch('/:id', requireAuth, async (req, res) => {
