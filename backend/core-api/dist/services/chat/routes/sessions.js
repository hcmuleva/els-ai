...
import { z } from 'zod';
import { requireAuth } from '../../auth/routes/auth.js';
...
chatSessionsRouter.post('/', requireAuth, async (req, res) => {
...
chatSessionsRouter.get('/:id', requireAuth, async (req, res) => {
    const user = req.user;
    if (!user?.organizationId)
        return res.status(401).json({ message: 'Unauthorized' });
    try {
        const sessionId = String(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
        const sessionRes = await db.query(`SELECT * FROM chat_sessions WHERE id = $1::uuid AND organization_id = $2::uuid LIMIT 1`, [sessionId, user.organizationId]);
...
chatSessionsRouter.get('/:id/messages', requireAuth, async (req, res) => {
    const user = req.user;
    if (!user?.organizationId)
        return res.status(401).json({ message: 'Unauthorized' });
    try {
        const sessionId = String(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
        const messagesRes = await db.query(`SELECT * FROM chat_messages
       WHERE session_id = $1::uuid AND organization_id = $2::uuid
       ORDER BY created_at ASC`, [sessionId, user.organizationId]);
...
chatSessionsRouter.post('/:id/messages', requireAuth, async (req, res) => {
...
chatSessionsRouter.post('/:id/realtime-token', requireAuth, async (req, res) => {
    const user = req.user;
    ...
    try {
        const sessionId = String(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
        const sessionRes = await db.query(`SELECT chat_type FROM chat_sessions WHERE id = $1::uuid AND organization_id = $2::uuid LIMIT 1`, [sessionId, user.organizationId]);
        const session = sessionRes.rows[0];
        if (!session)
            return res.status(404).json({ message: 'Session not found' });
...
chatSessionsRouter.patch('/:id/status', requireAuth, async (req, res) => {
    ...
    try {
        const sessionId = String(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
        const updateRes = await db.query(`UPDATE chat_sessions
       SET status = $1, updated_at = NOW()
       WHERE id = $2::uuid AND organization_id = $3::uuid
       RETURNING *`, [status, sessionId, user.organizationId]);
