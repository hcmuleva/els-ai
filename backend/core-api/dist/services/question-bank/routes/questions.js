...
questionsRouter.post('/', requireAuth, async (req, res) => {
    ...
    if (rawClassInput) {
        const classLookup = await db.query(`SELECT id, code FROM class_levels WHERE id::text = $1 OR code = $1 OR LOWER(label) = LOWER($1) LIMIT 1`, [rawClassInput]);
...
questionsRouter.patch('/:questionId', requireAuth, async (req, res) => {
    ...
    try {
        ...
        let resolvedClassLevel = undefined;
        const rawClassInput = (class_id || class_level || '').trim();
        if (rawClassInput) {
            const classLookup = await db.query(`SELECT id, code FROM class_levels WHERE id::text = $1 OR code = $1 OR LOWER(label) = LOWER($1) LIMIT 1`, [rawClassInput]);
            if (classLookup.rows.length > 0) {
        ...
        if (class_level !== undefined || subject !== undefined) {
            ...
            const baseQuestionData = questionData !== undefined && typeof questionData === 'object' && !Array.isArray(questionData)
                ...
                : existingQuestionData && typeof existingQuestionData === 'object' && !Array.isArray(existingQuestionData)
                    ...
                    : {};
            const rawMeta = baseQuestionData._meta;
            const existingMeta = rawMeta && typeof rawMeta === 'object' && !Array.isArray(rawMeta)
                ? rawMeta
                : {};
            preparedQuestionData = {
                ...baseQuestionData,
