...
async function getUserWithRoles(userId, organizationId) {
    ...
    const rolesResult = organizationId !== undefined
        ? await db.query(`SELECT r.role_name, ur.organization_id
           FROM roles r
           INNER JOIN user_roles ur ON ur.role_id = r.id
           WHERE ur.user_id = $1 AND ur.organization_id = $2
           ORDER BY r.role_name`, [userId, organizationId])
        : await db.query(`SELECT r.role_name, ur.organization_id
           FROM roles r
           INNER JOIN user_roles ur ON ur.role_id = r.id
           WHERE ur.user_id = $1
...
async function hasAdminAccess(req) {
    ...
    if (!userId || !organizationId)
        return false;
    const adminCheck = await db.query(`SELECT 1
     FROM users u
...
async function userHasRoleInOrg(userId, organizationId, roleName) {
    const result = await db.query(`SELECT 1
     FROM user_roles ur
     INNER JOIN roles r ON r.id = ur.role_id
     WHERE ur.user_id = $1
...
usersRouter.delete('/me', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (role === 'student') {
            const parentLink = await db.query(`SELECT 1 FROM parent_student_links WHERE student_user_id = $1 LIMIT 1`, [userId]);
...
usersRouter.post('/me/delete-child', requireAuth, async (req, res) => {
    ...
    try {
       ...
       LIMIT 1`, [parentUserId, registrationId.trim().toUpperCase()]);
        ...
        await db.query(`UPDATE users SET deleted_at = NOW(), is_active = false WHERE id = $1`, [childId]);
        await db.query(`DELETE FROM parent_student_links WHERE student_user_id = $1`, [childId]);
...
usersRouter.get('/admin/counts', requireAuth, async (req, res) => {
    ...
    try {
        const usersByRole = await db.query(`SELECT r.role_name AS role, COUNT(DISTINCT u.id)::int AS total
         FROM users u
        ...
        GROUP BY r.role_name`, [organizationId]);
        const counts = { students: 0, teachers: 0, parents: 0, admins: 0, subjects: 0 };
        usersByRole.rows.forEach((row) => {
            ...
            if (row.role === 'parent')
                counts.parents = Number(row.total) || 0;
            if (row.role === 'admin')
                counts.admins = Number(row.total) || 0;
        });
        const subjectsResult = await db.query(`SELECT COUNT(*)::int AS total
         FROM subjects s
        WHERE s.organization_id = $1::uuid`, [organizationId]);
...
usersRouter.get('/', requireAuth, async (req, res) => {
    ...
    if (search) {
        ...
        whereClauses.push(`(concat_ws(' ', u.first_name, u.last_name) ILIKE $${paramIndex}
    ...
    try {
        const usersResult = await db.query(`SELECT
          u.id,
          u.first_name,
          u.last_name,
          u.email,
...
usersRouter.post('/', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (mobileNumber) {
            const mobileExists = await db.query('SELECT 1 FROM users WHERE mobile_number = $1', [mobileNumber]);
...
usersRouter.patch('/:id/roles', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const membershipResult = await client.query(`SELECT 1
       FROM user_roles
...
usersRouter.patch('/:id', requireAuth, async (req, res) => {
    ...
    const membership = await db.query(`SELECT 1
     FROM user_roles
     WHERE user_id = $1
       AND organization_id = $2::uuid
     LIMIT 1`, [userId, organizationId]);
...
usersRouter.get('/students/search', requireAuth, async (req, res) => {
    ...
    const params = [organizationId];
    const whereClauses = ['ur.organization_id = $1::uuid', 'r.role_name = \'student\'', 'u.deleted_at IS NULL'];
    if (query) {
        params.push(`%${query}%`);
...
usersRouter.get('/parents/assignments', requireAuth, async (req, res) => {
    ...
    const params = [organizationId];
    const whereClauses = ['ur.organization_id = $1::uuid', 'r.role_name = \'parent\'', 'u.deleted_at IS NULL'];
    if (search) {
        params.push(`%${search}%`);
        const idx = params.length;
        whereClauses.push(`(concat_ws(' ', u.first_name, u.last_name) ILIKE $${idx}
...
usersRouter.put('/parents/:id/students', requireAuth, async (req, res) => {
    ...
    if (uniqueStudentIds.length > 0) {
        const studentValidation = await db.query(`SELECT COUNT(*)::int AS count
       FROM user_roles ur
       INNER JOIN roles r ON r.id = ur.role_id
       WHERE ur.organization_id = $1::uuid
         AND r.role_name = 'student'
         AND ur.user_id = ANY($2::uuid[])`, [organizationId, uniqueStudentIds]);
    ...
    try {
       ...
       WHERE parent_user_id = $1
         AND organization_id = $2::uuid`, [parentUserId, organizationId]);
        for (const studentId of uniqueStudentIds) {
            await client.query(`INSERT INTO parent_student_links (parent_user_id, student_user_id, organization_id)
         VALUES ($1, $2, $3::uuid)
         ON CONFLICT (parent_user_id, student_user_id, organization_id) DO NOTHING`, [parentUserId, studentId, organizationId]);
        }
        await client.query('COMMIT');
...
usersRouter.get('/teachers/assignments', requireAuth, async (req, res) => {
    ...
    const { search, limit } = parsedQuery.data;
    const params = [organizationId];
    const whereClauses = ['ur.organization_id = $1::uuid', 'r.role_name = \'teacher\'', 'u.deleted_at IS NULL'];
    if (search) {
        params.push(`%${search}%`);
        const idx = params.length;
        whereClauses.push(`(concat_ws(' ', u.first_name, u.last_name) ILIKE $${idx}
...
usersRouter.put('/teachers/:id/assignments', requireAuth, async (req, res) => {
    ...
    const uniqueClassAssignments = Array.from(new Map(parsedBody.data.classAssignments.map((item) => [item.classLevel.toLowerCase(), item])).values());
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        await client.query(`DELETE FROM teacher_class_assignments
       ...
       WHERE teacher_user_id = $1
        ...
        for (const ca of uniqueClassAssignments) {
            await client.query(`INSERT INTO teacher_class_assignments (teacher_user_id, organization_id, class_level, all_subjects)
         VALUES ($1::uuid, $2::uuid, $3::varchar, $4::boolean)
         ON CONFLICT (teacher_user_id, organization_id, class_level) DO NOTHING`, [teacherUserId, organizationId, ca.classLevel, ca.allSubjects]);
            if (!ca.allSubjects) {
             ...
             LIMIT 1
             ON CONFLICT (teacher_user_id, organization_id, class_level, subject_id) DO NOTHING`, [teacherUserId, organizationId, ca.classLevel, subject]);
                }
            }
        }
        await client.query('COMMIT');
...
usersRouter.get('/authors/search', requireAuth, async (req, res) => {
    ...
    const params = [organizationId];
    const whereClauses = ['ur.organization_id = $1::uuid', 'u.deleted_at IS NULL', 'u.is_active = true'];
    if (mobileNumber) {
        params.push(`%${mobileNumber}%`);
        whereClauses.push(`COALESCE(u.mobile_number, '') ILIKE $${params.length}`);
    }
...
usersRouter.get('/subjects', requireAuth, async (req, res) => {
    ...
    const params = [organizationId];
    const whereClauses = ['s.organization_id = $1::uuid'];
    if (search) {
        params.push(`%${search}%`);
        const idx = params.length;
        whereClauses.push(`(s.title ILIKE $${idx}
    ...
    try {
       ...
       LIMIT $${params.length}`, params);
        return res.json({
            subjects: result.rows.map((row) => mapSubjectRow(row)),
        });
    }
    catch (error) {
        console.error(error);
...
usersRouter.post('/subjects', requireAuth, async (req, res) => {
    ...
    try {
       ...
       LIMIT 1`, [organizationId, classLevel, title]);
        if ((duplicateCheck.rowCount ?? 0) > 0) {
            return res.status(400).json({ message: 'Subject already exists for this standard' });
        }
        if (authorUserId) {
            const authorExists = await db.query(`SELECT 1
         FROM user_roles ur
         WHERE ur.organization_id = $1::uuid
           AND ur.user_id = $2::uuid
         LIMIT 1`, [organizationId, authorUserId]);
       ...
       WHERE s.id = $1
       LIMIT 1`, [subjectId]);
        return res.status(201).json(mapSubjectRow(result.rows[0]));
    }
    catch (error) {
        console.error(error);
...
usersRouter.patch('/subjects/:id', requireAuth, async (req, res) => {
    ...
    const existingResult = await db.query(`SELECT
       id,
       title,
       class_level,
       author,
     ...
     WHERE id = $1
       AND organization_id = $2::uuid
     LIMIT 1`, [subjectId, organizationId]);
    ...
    const duplicateCheck = await db.query(`SELECT 1 FROM subjects
     WHERE organization_id = $1::uuid
       ...
       AND LOWER(title) = LOWER($3)
       AND id <> $4
     LIMIT 1`, [organizationId, effectiveClassLevel, effectiveTitle, subjectId]);
    if ((duplicateCheck.rowCount ?? 0) > 0) {
        return res.status(400).json({ message: 'Subject already exists for this standard' });
    }
    let nextIsExternalAuthor = existing.is_external_author ?? false;
    if (parsedBody.data.isExternalAuthor !== undefined) {
        nextIsExternalAuthor = parsedBody.data.isExternalAuthor;
    }
    let nextAuthorUserId = existing.author_user_id || null;
    let nextAuthorName = existing.author || null;
    ...
    if (nextAuthorUserId) {
       ...
       WHERE ur.organization_id = $1::uuid
         AND ur.user_id = $2::uuid
       LIMIT 1`, [organizationId, nextAuthorUserId]);
        if ((authorExists.rowCount ?? 0) === 0) {
            return res.status(400).json({ message: 'Selected author does not belong to your organization' });
        }
    }
    ...
    try {
        await db.query(`UPDATE subjects
       SET ${updates.join(', ')}
       ...
       WHERE s.id = $1
       LIMIT 1`, [subjectId]);
        return res.json(mapSubjectRow(result.rows[0]));
    }
    catch (error) {
        console.error(error);
...
usersRouter.delete('/subjects/:id', requireAuth, async (req, res) => {
    const subjectId = getSingleParam(req.params.id);
    if (!subjectId) {
        return res.status(400).json({ message: 'Invalid subject id' });
    }
    ...
    try {
        await client.query('BEGIN');
        const subjectResult = await client.query(`SELECT class_level, title
       FROM subjects
       WHERE id = $1
         AND organization_id = $2::uuid
       LIMIT 1`, [subjectId, organizationId]);
...
usersRouter.patch('/:id/global-publish-permission', requireAuth, async (req, res) => {
    const userId = getSingleParam(req.params.id);
    ...
    try {
        await db.query(`INSERT INTO user_global_publish_permissions (user_id, organization_id, enabled, granted_by, granted_at, updated_at)
       VALUES ($1, $2::uuid, $3, $4, NOW(), NOW())
       ON CONFLICT (user_id, organization_id)
       DO UPDATE SET enabled = EXCLUDED.enabled, granted_by = EXCLUDED.granted_by, updated_at = NOW()`, [userId, organizationId, parsedBody.data.enabled, req.user?.userId || null]);
...
usersRouter.patch('/:id/organization-membership', requireAuth, async (req, res) => {
    const userId = getSingleParam(req.params.id);
    ...
    try {
        ...
        const existingRolesResult = await client.query(`SELECT r.role_name
       FROM user_roles ur
       INNER JOIN roles r ON r.id = ur.role_id
       WHERE ur.user_id = $1
         AND ur.organization_id = $2::uuid`, [userId, fromOrganizationId]);
        if ((existingRolesResult.rowCount ?? 0) === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ message: 'User membership not found in source organization' });
        }
        const rolesToAssign = selectedRoles || existingRolesResult.rows.map((row) => row.role_name);
        await client.query(`DELETE FROM user_roles
       WHERE user_id = $1
         AND organization_id = $2::uuid`, [userId, fromOrganizationId]);
...
usersRouter.patch('/:id/active-role', requireAuth, async (req, res) => {
    const userId = getSingleParam(req.params.id);
    ...
    try {
       ...
       WHERE ur.user_id = $1 AND r.role_name = $2
         ${organizationId ? 'AND ur.organization_id = $3' : ''}`, organizationId ? [userId, roleName, organizationId] : [userId, roleName]);
       ...
       WHERE id = $2`, [roleName, userId]);
        ...
        const globalPublishPermissionResult = updatedUser.organizationId
            ? await db.query(`SELECT enabled
             FROM user_global_publish_permissions
             WHERE user_id = $1
               AND organization_id = $2::uuid
             LIMIT 1`, [updatedUser.id, updatedUser.organizationId])
            : { rows: [] };
