...
import { enforceSubscriptionState, getCurrentOrganizationSubscription, isSubscriptionActive } from '../services/billing.js';
import { eventBus } from '../events/bus.js';

async function isAdminOrSuper(userId: string | undefined, organizationId: string): Promise<{ ok: boolean; superAdmin: boolean }> {
  if (!userId) return { ok: false, superAdmin: false };
  const result = await db.query(
    `SELECT r.role_name
     FROM user_roles ur
     INNER JOIN roles r ON r.id = ur.role_id
     WHERE ur.user_id = $1 AND (ur.organization_id = $2::uuid OR r.role_name = 'superadmin')`,
    [userId, organizationId],
  );
  const roles = result.rows.map((row: any) => row.role_name as string);
...
function mapInvoiceRow(row: any) {
  return {
    ...
    billingKind: row.billing_kind as string,
    planName: (row.plan_name as string | null) || undefined,
    membershipTier: (row.membership_tier as string | null) || undefined,
    billingCycle: (row.billing_cycle as string | null) || undefined,
    ...
    organizationSubdomain: (row.organization_subdomain as string | null) || undefined,
  };
}
...
function getCycleInterval(cycle: string): string {
...
billingRouter.get('/plans', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  return res.json({
    plans: result.rows.map((row) => ({
      ...
      membershipTier: row.membership_tier as string,
...
billingRouter.post('/plans', requireAuth, async (req: AuthenticatedRequest, res) => {
...
billingRouter.patch('/plans/:planId', requireAuth, async (req: AuthenticatedRequest, res) => {
...
billingRouter.get('/organizations/:organizationId/subscription', requireAuth, async (req: AuthenticatedRequest, res) => {
...
billingRouter.post('/organizations/:organizationId/subscribe', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = req.params.organizationId as string;
  ...
  try {
    const planResult = await db.query(
      `SELECT id, billing_cycle, base_price, offer_discount_percent, special_discount_percent, group_discount_percent, max_users_for_group_discount
       FROM subscription_plans
       WHERE id = $1::uuid
         AND is_active = true
       LIMIT 1`,
    ...
    const created = await db.query(
      `INSERT INTO organization_subscriptions (
       ...
       RETURNING *`,
      [organizationId, planId, getCycleInterval(plan.billing_cycle as string), finalPrice, seatCount, offer, special, groupDiscount],
    );

    const subscriptionRow = created.rows[0];
    const planDetails = await db.query(
      `SELECT name, membership_tier, billing_cycle FROM subscription_plans WHERE id = $1::uuid LIMIT 1`,
      [planId],
    );
...
billingRouter.post('/organizations/:organizationId/trial/reset', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = req.params.organizationId as string;
  if (!organizationId) {
    return res.status(400).json({ message: 'Invalid organization id' });
  }
...
billingRouter.get('/organizations/:organizationId/invoices', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = req.params.organizationId as string;
  if (!organizationId) {
    return res.status(400).json({ message: 'Invalid organization id' });
  }
  ...
  const where: string[] = [`i.organization_id = $1::uuid`];
  ...
  const result = await db.query(
    `SELECT i.*, o.name AS organization_name, o.subdomain AS organization_subdomain
     FROM invoices i
...
billingRouter.get('/invoices/pending', requireAuth, async (req: AuthenticatedRequest, res) => {
  if (!(await isSuperAdmin(req.user?.userId))) {
    return res.status(403).json({ message: 'Forbidden: superadmin role required' });
  }
...
billingRouter.get('/invoices/:invoiceId', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  if ((result.rowCount ?? 0) === 0) return res.status(404).json({ message: 'Invoice not found' });
...
billingRouter.post('/invoices/:invoiceId/pay', requireAuth, async (req: AuthenticatedRequest, res) => {
  const invoiceId = req.params.invoiceId as string;
  if (!invoiceId) return res.status(400).json({ message: 'Invalid invoice id' });
  ...
  if ((lookup.rowCount ?? 0) === 0) return res.status(404).json({ message: 'Invoice not found' });
  const row = lookup.rows[0];
  if (row.status === 'paid') return res.status(400).json({ message: 'Invoice already marked paid' });
  ...
  const result = await db.query(
    `UPDATE invoices
     ...
     WHERE id = $1::uuid
     RETURNING *`,
    [invoiceId, parsed.data.paymentMethod, paymentReference, parsed.data.notes || null],
  );
  ...
  if (row.subscription_id) {
    await db.query(
      `UPDATE organization_subscriptions
       ...
       WHERE id = $1::uuid AND status IN ('trialing', 'past_due', 'expired')`,
      [row.subscription_id],
    );
  }
...
billingRouter.post('/organizations/:organizationId/renew', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = req.params.organizationId as string;
  if (!organizationId) return res.status(400).json({ message: 'Invalid organization id' });

  const access = await isAdminOrSuper(req.user?.userId, organizationId);
  if (!access.ok) return res.status(403).json({ message: 'Forbidden: admin or superadmin required' });

  const current = await db.query(
    `SELECT s.*, p.name AS plan_name, p.membership_tier, p.billing_cycle, p.base_price
  ...
  await db.query(
    `UPDATE organization_subscriptions
     SET status = 'expired', updated_at = NOW()
     WHERE organization_id = $1::uuid AND status IN ('trialing', 'active', 'past_due')`,
    [organizationId],
  );

  await db.query(
    `UPDATE invoices SET status = 'renewed', updated_at = NOW()
     WHERE organization_id = $1::uuid AND status = 'pending'`,
    [organizationId],
  );

  const interval = getCycleInterval(sub.billing_cycle as string);
...
billingRouter.post('/organizations/:organizationId/deactivate', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = req.params.organizationId as string;
  if (!organizationId) return res.status(400).json({ message: 'Invalid organization id' });
  const access = await isAdminOrSuper(req.user?.userId, organizationId);
  if (!access.ok) return res.status(403).json({ message: 'Forbidden: admin or superadmin required' });
