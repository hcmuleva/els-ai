...
const EXTRA_ACTIVITY_SUBJECTS: SubjectTemplate[] = [
...
async function seedTopicsAndContent(orgId: string) {
  ...
  await db.query(
    `INSERT INTO learning_content_sections (content_id, section_order, content_type, text_content)
     VALUES ($1, 1, 'text', $2)`,
    [c11Id, "Welcome to Phonics! Let's learn the sounds of English letters. 'A' says /æ/ as in Apple. 'B' says /b/ as in Ball. 'C' says /k/ as in Cat."]
  );
  ...
  await db.query(
    `INSERT INTO learning_content_sections (content_id, section_order, content_type, media_url, text_content)
     VALUES ($1, $2, $3, $4, $5)`,
    [c12Id, 1, 'audio', '/media/bg-audio/eliveta-kids-happy-music-474162.mp3', 'Listen to the sounds of the letters and repeat them aloud.']
  );
  ...
  await db.query(
    `INSERT INTO learning_content_sections (content_id, section_order, content_type, text_content)
     VALUES ($1, 1, 'text', $2)`,
    [c21Id, 'A noun is a naming word. It names a person, place, animal, or thing. Examples: Boy (person), School (place), Dog (animal), Toy (thing).']
  );
  ...
  await db.query(
    `INSERT INTO learning_content_sections (content_id, section_order, content_type, text_content)
     VALUES ($1, 1, 'text', $2)`,
    [c31Id, 'Action words tell us what someone or something is doing. Examples: run, jump, read, write, play, sleep.']
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_audio, question_data, sort_order, points)
     VALUES ($1, 'guess_audio', 'Listen to the sound', 'Which word matches the sound you hear?', '/media/sounds/lion.mp3', $2, 1, 10)`,
    [q1Id, JSON.stringify(q1q1Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_audio, question_data, sort_order, points)
     VALUES ($1, 'guess_audio', 'Listen carefully', 'Select the correct name of the animal.', '/media/sounds/dog.mp3', $2, 2, 10)`,
    [q1Id, JSON.stringify(q1q2Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'guess_image', 'Identify the animal', 'What is the name of this animal?', $2, 1, 10)`,
    [q2Id, JSON.stringify(q2q1Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'guess_image', 'Name this bird', 'Choose the correct name for this bird.', $2, 2, 10)`,
    [q2Id, JSON.stringify(q2q2Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'guess_image', 'Identify the Action', 'What action does the rooster do in the morning?', $2, 1, 10)`,
    [q3Id, JSON.stringify(q3q1Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'guess_image', 'Look at the picture', 'Choose the correct animal name.', $2, 1, 10)`,
    [q4Id, JSON.stringify(masterQ1Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_audio, question_data, sort_order, points)
     VALUES ($1, 'guess_audio', 'Listen carefully', 'Which animal name matches the sound?', '/media/sounds/lion.mp3', $2, 2, 10)`,
    [q4Id, JSON.stringify(masterQ2Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'true_false', 'Is a dog a pet animal?', 'Choose True or False.', $2, 3, 10)`,
    [q4Id, JSON.stringify(masterQ3Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'single_choice', 'Which word starts with the letter A?', 'Choose the correct option.', $2, 4, 10)`,
    [q4Id, JSON.stringify(masterQ4Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'multi_choice', 'Identify the Vowels', 'Select all the vowels from the list below.', $2, 5, 10)`,
    [q4Id, JSON.stringify(masterQ5Data)]
  );
  ...
  await db.query(
    `INSERT INTO quiz_questions (quiz_id, question_type, question_title, question_instruction, question_data, sort_order, points)
     VALUES ($1, 'drag_drop_match', 'Match Animals to Homes', 'Drag each animal to where it lives!', $2, 6, 10)`,
    [q4Id, JSON.stringify(masterQ6Data)]
  );
