...
type Profile = {
  ...
  parentLast: string;
  parentEmail: string;
  // score model (percent 0-100). value = base + slope * t^curve + season + noise
  // `curve` > 1 keeps early gains small, then accelerates (the classic
  // "weak start → small improvement → big growth after a year" arc).
  base: number;
  slope: number;
  curve: number;
  noise: number;
...
function scoreFor(p: Profile, t: number, salt: number): number {
...
const pctTo5 = (pct: number) => clamp(Math.round(pct / 20), 1, 5);
...
async function resolveOrgId(): Promise<string | null> {
  ...
  if ((def.rowCount ?? 0) > 0) return def.rows[0].id as string;
  const els = await db.query(`SELECT id FROM organizations WHERE subdomain = 'els-academy' LIMIT 1`);
  return (els.rowCount ?? 0) > 0 ? (els.rows[0].id as string) : null;
}
...
async function upsertUser(
  ...
  if ((existing.rowCount ?? 0) > 0) {
    ...
    await db.query(
      `UPDATE users SET first_name=$2, last_name=$3, password_hash=$4, active_role=$5, class_level=$6, is_active=true, updated_at=NOW() WHERE id=$1`,
      [userId, first, last, passwordHash, role, classLevel],
    );
  } else {
    const created = await db.query(
      `INSERT INTO users(first_name, last_name, email, password_hash, active_role, class_level)
       VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,
      [first, last, email, passwordHash, role, classLevel],
    );
  ...
  if ((roleRow.rowCount ?? 0) > 0) {
    await db.query(
      `INSERT INTO user_roles(user_id, role_id, organization_id)
       VALUES($1,$2,$3) ON CONFLICT (user_id, role_id, organization_id) DO NOTHING`,
      [userId, roleRow.rows[0].id, orgId],
    );
  }
  return userId;
}

async function deleteDemoAccounts(teacherEmails: string[]) {
  const allEmails = [
  ...
  await db.query(
    `DELETE FROM classrooms WHERE created_by IN (SELECT id FROM users WHERE email = ANY($1::text[]))`,
    [teacherEmails],
  );
  await db.query(`DELETE FROM users WHERE email = ANY($1::text[])`, [allEmails]);
}

async function getOrCreateAchievements(orgId: string): Promise<Array<{ id: string }>> {
  let rows = (
    await db.query(`SELECT id FROM achievements WHERE is_global = true ORDER BY created_at NULLS LAST LIMIT 8`)
  ).rows as Array<{ id: string }>;
  if (rows.length === 0) {
    rows = (
      await db.query(`SELECT id FROM achievements WHERE organization_id=$1::uuid LIMIT 8`, [orgId])
    ).rows as Array<{ id: string }>;
  }
  if (rows.length === 0) {
    ...
    for (const [name, emoji, color] of demo) {
      try {
        const r = await db.query(
          `INSERT INTO achievements (organization_id, name, emoji, color, description, is_global)
           VALUES ($1,$2,$3,$4,$5,false) RETURNING id`,
          [orgId, name, emoji, color, `${name} award`],
        );
        rows.push({ id: r.rows[0].id as string });
      } catch {
...
export async function seedDemoTrends(orgIdArg?: string): Promise<void> {
  ...
  const sharedParentId = await upsertUser(
    orgId, passwordHash, SHARED_PARENT.first, SHARED_PARENT.last, SHARED_PARENT.email, 'parent', null,
  );
  ...
  const quizRows = (
    await db.query(`SELECT id FROM quizzes WHERE organization_id=$1::uuid ORDER BY created_at LIMIT 30`, [orgId])
  ).rows as Array<{ id: string }>;
  ...
  for (let pi = 0; pi < PROFILES.length; pi++) {
    ...
    await db.query(
      `INSERT INTO parent_student_links(parent_user_id, student_user_id, organization_id)
       VALUES($1,$2,$3) ON CONFLICT (parent_user_id, student_user_id, organization_id) DO NOTHING`,
    ...
    await db.query(
      `INSERT INTO parent_student_links(parent_user_id, student_user_id, organization_id)
       VALUES($1,$2,$3) ON CONFLICT (parent_user_id, student_user_id, organization_id) DO NOTHING`,
      [sharedParentId, studentId, orgId],
    );
    ...
    for (let dN = 0; dN <= days; dN++) {
      ...
      if (!dense && (dow === 0 || dow === 6) && rand(salt) > 0.35) continue;
      const t = dN / days;
      ...
      if (dense || rand(salt + 1) > 0.15) {
        ...
        await db.query(
          `INSERT INTO student_activity(student_id, organization_id, activity_type, reference_title, status, score, time_spent_seconds, activity_date, created_at)
           VALUES($1,$2,'content',$3,'completed',NULL,$4,$5::date,$6)`,
          [studentId, orgId, `${p.subject} lesson`, ts, dateStr, date],
        );
        counts.activity++;
        dayAttempted++; dayCompleted++; dayTime += ts;
      }
      ...
      if (dense || rand(salt + 3) > 0.55) {
        ...
        await db.query(
          `INSERT INTO student_activity(student_id, organization_id, activity_type, reference_title, status, score, time_spent_seconds, activity_date, created_at)
           VALUES($1,$2,'quiz',$3,$4,$5,$6,$7::date,$8)`,
          [studentId, orgId, `${p.subject} quiz`, isDone ? 'completed' : 'attempted', qscore, ts, dateStr, date],
        );
        counts.activity++;
        dayAttempted++; if (isDone) dayCompleted++; dayTime += ts;
        ...
        if (dense && quizRows.length > 0) {
          ...
          await db.query(
            `INSERT INTO student_attempts(student_id, quiz_id, score, total_points, completed_at)
             VALUES($1,$2,$3,100,$4)`,
            [studentId, quizId, qscore, date],
          );
          counts.attempts++;
        }
      }
      ...
      if (rand(salt + 7) > 0.86) {
        ...
        await db.query(
          `INSERT INTO student_activity(student_id, organization_id, activity_type, reference_title, status, score, time_spent_seconds, activity_date, created_at)
           VALUES($1,$2,'assignment',$3,'completed',$4,$5,$6::date,$7)`,
          [studentId, orgId, `${p.subject} worksheet`, scoreFor(p, t, salt + 8), ts, dateStr, date],
        );
        counts.activity++;
        dayAttempted++; dayCompleted++; dayTime += ts;
      }
      ...
      if (dayAttempted > 0) {
        ...
        await db.query(
          `INSERT INTO student_analytics(student_id, organization_id, analytics_date, streak_days, consistency_score, attempted_count, not_attempted_count, completed_count, completion_rate, total_time_seconds, created_at)
    ...
    for (let i = 0; i <= TERMS; i++) {
      ...
      const classroom = await db.query(
        `INSERT INTO classrooms(organization_id, title, description, schedule_type, start_time, end_time, duration_minutes, class_level, created_by, status, is_global, created_at, updated_at, ended_at)
         VALUES($1,$2,$3,'instant',NULL,NULL,45,$4,$5,$6,false,$7,$7,$8) RETURNING id`,
        [orgId, `${p.subject} - Term ${i + 1}`, `${p.subject} classroom for Class ${p.classLevel}`, p.classLevel, teacherId, status, created, ended],
      );
      ...
      await db.query(
        `INSERT INTO classroom_student_remarks(classroom_id, student_id, teacher_id, remark_text, parent_note, remark_media_url, score_behavior, score_confidence, score_participation, score_performance, created_at, updated_at)
      ...
      if (achievements.length > 0 && pct >= 72 && !isActive) {
        ...
        try {
          await db.query(
            `INSERT INTO student_achievements(student_id, classroom_id, achievement_id, granted_by, granted_at)
             VALUES($1,$2,$3,$4,$5)
             ON CONFLICT (student_id, classroom_id, achievement_id) DO NOTHING`,
            [studentId, classroomId, achId, teacherId, remarkDate],
          );
          counts.achievements++;
        } catch {
    ...
    for (let s = 0; s < SESSIONS; s++) {
      ...
      const session = await db.query(
        `INSERT INTO counseling_sessions(parent_user_id, student_user_id, organization_id, status, student_snapshot, duration_sec, started_at, submitted_at, created_at, updated_at)
         VALUES($1,$2,$3,'reported',$4::jsonb,$5,$6,$7,$6,$7) RETURNING id`,
        [parentId, studentId, orgId, snapshot, 1500, when, submitted],
      );
      const sessionId = session.rows[0].id as string;

      await db.query(
        `INSERT INTO counseling_reports(session_id, student_user_id, organization_id, overall_score, level, growth_potential, study_pattern_type, report_json, created_at)
    ...
    for (let f = 0; f <= FEEDBACK; f++) {
      ...
      await db.query(
        `INSERT INTO parent_feedback(parent_user_id, student_user_id, organization_id, feedback_text, attachment_url, created_at)
         VALUES($1,$2,$3,$4,NULL,$5)`,
        [parentId, studentId, orgId, pick(pool, f), when],
      );
      counts.feedback++;
    }
    ...
    for (let a = 0; a <= TERMS; a++) {
      ...
      const t = a / TERMS;
      const salt = pi * 40 + a;
      const to10 = (sl: number) => clamp(Math.round(scoreFor(p, t, sl) / 10), 0, 10);
      await db.query(
        `INSERT INTO parent_assessments(parent_user_id, student_user_id, organization_id, behavior_score, focus_score, regularity_score, creativity_score, academic_score, outdoor_activity_score, created_at, updated_at)
