...
async function tableExists(table) {
    const result = await db.query(`SELECT 1 FROM information_schema.tables WHERE table_name = $1`, [table]);
    return (result.rowCount ?? 0) > 0;
}
...
async function consolidateDefaultOrg() {
    const elsAcademy = await db.query(`SELECT id FROM organizations WHERE subdomain = 'els-academy' LIMIT 1`);
    let canonicalId = elsAcademy.rows[0]?.id;
    if (!canonicalId) {
        const inserted = await db.query(`INSERT INTO organizations (name, subdomain, settings, is_default)
    ...
    const legacy = await db.query(`SELECT id FROM organizations WHERE subdomain = 'default-org' LIMIT 1`);
    const legacyId = legacy.rows[0]?.id;
    if (legacyId && legacyId !== canonicalId) {
        for (const table of TENANT_TABLES) {
            if (!(await tableExists(table)))
                continue;
            if (!(await tableHasColumn(table, 'organization_id')))
                continue;
            try {
                await db.query(`UPDATE ${table} SET organization_id = $1::uuid WHERE organization_id = $2::uuid`, [canonicalId, legacyId]);
            }
            catch (err) {
                const code = err.code;
                if (code === '23505') {
                    await db.query(`DELETE FROM ${table} WHERE organization_id = $1::uuid`, [legacyId]);
                }
                else {
                    throw err;
                }
    ...
    await db.query(`UPDATE organizations SET is_default = false WHERE id <> $1::uuid AND is_default = true`, [canonicalId]);
...
async function backfillTenantTables(defaultOrgId) {
    for (const table of TENANT_TABLES) {
        ...
        if (!(await tableHasColumn(table, 'organization_id')))
            continue;
        try {
            await db.query(`UPDATE ${table} SET organization_id = $1::uuid WHERE organization_id IS NULL`, [defaultOrgId]);
        }
        catch (err) {
            console.warn(`[org-service migrate] could not backfill ${table}:`, err.message);
        }
    }
}
