...
async function isOrgMember(userId: string | undefined, orgId: string): Promise<boolean> {
  if (!userId) return false;
  const result = await db.query(
    `SELECT 1 FROM user_org_mapping WHERE user_id = $1 AND organization_id = $2::uuid LIMIT 1`,
    [userId, orgId],
  );
  return (result.rowCount ?? 0) > 0;
}

async function isOrgAdminOrSuper(req: AuthenticatedRequest, orgId: string): Promise<boolean> {
...
function rowToOrg(row: any) {
  return {
    ...
    subdomain: (row.subdomain as string | null) || undefined,
    logo: (row.logo as string | null) || (row.logo_url as string | null) || undefined,
...
organizationsRouter.get('/_meta', requireAuth, async (_req: AuthenticatedRequest, res) => {
...
organizationsRouter.get('/me', requireAuth, async (req: AuthenticatedRequest, res) => {
  const userId = req.user?.userId;
  if (!userId) return res.status(401).json({ message: 'Authentication required' });
  const result = await db.query(
    `SELECT o.id, o.name, o.subdomain, o.logo, o.logo_url, o.is_default, o.settings, o.created_at, o.updated_at, uom.is_primary
     FROM user_org_mapping uom
     INNER JOIN organizations o ON o.id = uom.organization_id
     WHERE uom.user_id = $1
       AND o.deleted_at IS NULL
     ORDER BY uom.is_primary DESC, o.name ASC`,
    [userId],
  );
  res.json({
...
organizationsRouter.get('/', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  const result = await db.query(
    `SELECT id, name, subdomain, logo, logo_url, is_default, settings, created_at, updated_at, deleted_at
     FROM organizations
...
organizationsRouter.get('/:id', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  if (!canRead) return res.status(403).json({ message: 'Forbidden: not a member of this organization' });
  ...
  if ((result.rowCount ?? 0) === 0) return res.status(404).json({ message: 'Organization not found' });
  return res.json(rowToOrg(result.rows[0]));
});

organizationsRouter.post('/', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO organizations (name, subdomain, logo, logo_url, is_default, settings)
         VALUES ($1, LOWER($2), $3, $4, COALESCE($5, false), $6)
         RETURNING id, name, subdomain, logo, logo_url, is_default, settings, created_at, updated_at, deleted_at`,
        [name, subdomain, logo ?? logoUrl ?? null, logoUrl ?? null, isDefault ?? false, settings || {}],
      );
...
organizationsRouter.patch('/:id', requireAuth, async (req: AuthenticatedRequest, res) => {
  const orgId = req.params.id as string;
...
organizationsRouter.delete('/:id', requireAuth, async (req: AuthenticatedRequest, res) => {
  const orgId = req.params.id as string;
  if (!orgId) return res.status(400).json({ message: 'Invalid organization id' });
  if (!isSuperAdmin(req)) {
    return res.status(403).json({ message: 'Forbidden: superadmin role required' });
  ...
  const existing = await db.query<{ is_default: boolean }>(
    `SELECT is_default FROM organizations WHERE id = $1::uuid AND deleted_at IS NULL`,
    [orgId],
  );
  if ((existing.rowCount ?? 0) === 0) return res.status(404).json({ message: 'Organization not found' });
  if (existing.rows[0].is_default) {
    return res.status(409).json({ message: 'Cannot delete the default organization' });
  }

  await db.query(
    `UPDATE organizations SET deleted_at = NOW(), updated_at = NOW() WHERE id = $1::uuid`,
    [orgId],
  );
  return res.status(204).send();
});

organizationsRouter.get('/:id/users', requireAuth, async (req: AuthenticatedRequest, res) => {
  const orgId = req.params.id as string;
  ...
  if (!allowed) return res.status(403).json({ message: 'Forbidden: not a member of this organization' });

  const { search, role } = parsedQuery.data;
  const params: unknown[] = [orgId];
  const where: string[] = ['uom.organization_id = $1::uuid', 'u.deleted_at IS NULL'];
  ...
  try {
    ...
    return res.json({
      users: result.rows.map((row) => ({
        ...
        activeRole: row.active_role as string,
...
organizationsRouter.post('/:id/members', requireAuth, async (req: AuthenticatedRequest, res) => {
  const orgId = req.params.id as string;
  ...
  try {
    ...
    await client.query(
      ...
      [userId, orgId, isPrimary ?? false],
    );
    if (isPrimary === true) {
      await client.query(
        `UPDATE user_org_mapping SET is_primary = false WHERE user_id = $1::uuid AND organization_id <> $2::uuid`,
        [userId, orgId],
      );
      await client.query(
        `UPDATE user_org_mapping SET is_primary = true WHERE user_id = $1::uuid AND organization_id = $2::uuid`,
        [userId, orgId],
      );
      await client.query(
        `UPDATE users SET primary_organization_id = $2::uuid WHERE id = $1::uuid`,
        [userId, orgId],
      );
    }
    if (roleName) {
...
organizationsRouter.delete('/:id/members/:userId', requireAuth, async (req: AuthenticatedRequest, res) => {
  const orgId = req.params.id as string;
  const memberId = req.params.userId as string;
  if (!orgId || !memberId) return res.status(400).json({ message: 'Invalid params' });
  if (!(await isOrgAdminOrSuper(req, orgId))) {
    return res.status(403).json({ message: 'Forbidden: admin/superadmin role required' });
  }
  const client = await db.connect();
  try {
    await client.query('BEGIN');
    await client.query(
      `DELETE FROM user_roles WHERE user_id = $1::uuid AND organization_id = $2::uuid`,
      [memberId, orgId],
    );
    await client.query(
      `DELETE FROM user_org_mapping WHERE user_id = $1::uuid AND organization_id = $2::uuid`,
      [memberId, orgId],
    );
    await client.query(
      `UPDATE users SET primary_organization_id = NULL WHERE id = $1::uuid AND primary_organization_id = $2::uuid`,
      [memberId, orgId],
    );
    await client.query('COMMIT');
    return res.status(204).send();
...
organizationsRouter.patch('/:id/members/:userId/primary', requireAuth, async (req: AuthenticatedRequest, res) => {
  const orgId = req.params.id as string;
  const memberId = req.params.userId as string;
  if (!orgId || !memberId) return res.status(400).json({ message: 'Invalid params' });
  ...
  try {
    ...
    const exists = await client.query(
      `SELECT 1 FROM user_org_mapping WHERE user_id = $1::uuid AND organization_id = $2::uuid LIMIT 1`,
      [memberId, orgId],
    );
    ...
    await client.query(
      `UPDATE user_org_mapping SET is_primary = false WHERE user_id = $1::uuid`,
      [memberId],
    );
    await client.query(
      `UPDATE user_org_mapping SET is_primary = true WHERE user_id = $1::uuid AND organization_id = $2::uuid`,
