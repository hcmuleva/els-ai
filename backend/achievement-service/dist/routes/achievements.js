...
import { canManageTeacherContent, getUserId, } from '@els-ai/internal-auth';
import { db } from '../db.js';
import { requireAuth } from '../middleware/auth.js';
export const achievementsRouter = Router();
achievementsRouter.get('/_meta', requireAuth, async (_req, res) => {
    const totals = await db.query(`SELECT COUNT(*)::text AS count FROM achievements WHERE is_global = true`);
    res.json({
...
achievementsRouter.get('/', requireAuth, async (_req, res) => {
    try {
...
achievementsRouter.post('/grant', requireAuth, async (req, res) => {
...
achievementsRouter.get('/my', requireAuth, async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return res.status(400).json({ message: 'Auth context missing' });
    try {
