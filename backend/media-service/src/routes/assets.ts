...
assetsRouter.post('/upload', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const insertResult = await db.query(
      `INSERT INTO assets (organization_id, asset_type, file_url, metadata)
       VALUES ($1::uuid, $2, $3, $4::jsonb)
       RETURNING id`,
      [organizationId, mediaType, uploaded.canonicalUrl, metadata],
    );
...
assetsRouter.post('/resolve', requireAuth, async (req: AuthenticatedRequest, res) => {
...
assetsRouter.post('/resolve/batch', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const resolved = await Promise.all(
      parsedBody.data.urls.map(async (inputUrl) => {
...
assetsRouter.get('/:assetId', requireAuth, async (req: AuthenticatedRequest, res) => {
  const organizationId = getOrganizationId(req);
  if (!organizationId) {
    return res.status(400).json({ message: 'Organization not found in auth context' });
  }
  ...
  try {
    const result = await db.query(
      `SELECT id, organization_id, asset_type, file_url, metadata, created_at
       FROM assets
    ...
    const canonicalUrl = toPersistentMediaUrl(asset.file_url as string);
    const signedUrl = await getSignedMediaUrlIfNeeded(canonicalUrl);
    return res.json({
      id: asset.id,
...
internalAssetsRouter.post('/upload', requireInternalSecret, async (req, res) => {
  ...
  try {
    ...
    const metadata = {
      ...
      originalFileName: fileName,
      storedFileName: uploaded.fileName,
      uploadedBy: (req.headers['x-internal-user-id'] as string | undefined) || null,
      context: null,
    };
