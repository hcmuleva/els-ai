...
quizzesRouter.get('/teacher/library', requireAuth, async (req, res) => {
    ...
    params.push(limit);
    try {
        const result = await db.query(`SELECT id, title, description, class_level, subject, quiz_type, difficulty_level, total_questions, is_published, is_ai_generated, is_global, created_at
       FROM quizzes
...
quizzesRouter.get('/:id', requireAuth, async (req, res) => {
    ...
    try {
        const quizResult = await db.query(`SELECT * FROM quizzes WHERE id = $1 AND (organization_id = $2::uuid OR is_global = true)`, [id, orgId]);
        if ((quizResult.rowCount ?? 0) === 0) {
            return res.status(404).json({ message: 'Quiz not found' });
        }
...
quizzesRouter.patch('/:id/publish', requireAuth, async (req, res) => {
    ...
    try {
       ...
       RETURNING id, title, class_level, subject, quiz_type, difficulty_level, total_questions, is_published, is_ai_generated, is_global, created_at, updated_at`, [parsed.data.isPublished, id, orgId]);
