...
quizzesRouter.get('/teacher/library', requireAuth, async (req, res) => {
    ...
    const params = [orgId];
    const whereClauses = ['(q.organization_id = $1::uuid OR q.is_global = true)'];
    if (search) {
        params.push(`%${search}%`);
    ...
    params.push(limit);
    try {
        const result = await db.query(`SELECT q.id, q.title, q.description, q.class_level, s.title AS subject, q.quiz_type, q.difficulty_level, q.total_questions, q.is_published, q.is_ai_generated, q.is_global, q.created_at
       FROM quizzes q
       LEFT JOIN subjects s ON s.id = q.subject_id
...
quizzesRouter.get('/teacher/overview', requireAuth, async (req, res) => {
    ...
    try {
       ...
       LIMIT 6`, [orgId]);
        const recentQuizzesResult = await db.query(`SELECT q.id, q.title, q.class_level, s.title AS subject, q.quiz_type, q.difficulty_level, q.total_questions, q.is_published, q.is_ai_generated, q.created_at
       FROM quizzes q
       LEFT JOIN subjects s ON s.id = q.subject_id
       WHERE q.organization_id = $1::uuid
       ORDER BY q.created_at DESC
...
quizzesRouter.get('/:id', requireAuth, async (req, res) => {
    ...
    try {
        const quizResult = await db.query(`SELECT q.*, s.title AS subject
       FROM quizzes q
       LEFT JOIN subjects s ON s.id = q.subject_id
       WHERE q.id = $1 AND (q.organization_id = $2::uuid OR q.is_global = true)`, [id, orgId]);
        if ((quizResult.rowCount ?? 0) === 0) {
            return res.status(404).json({ message: 'Quiz not found' });
        }
...
quizzesRouter.patch('/:id/publish', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const result = await db.query(`UPDATE quizzes q
       SET is_published = $1, updated_at = NOW()
       WHERE q.id = $2 AND (q.organization_id = $3::uuid OR q.is_global = true)
       RETURNING q.id, q.title, q.class_level, (SELECT title FROM subjects WHERE id = q.subject_id) AS subject, q.quiz_type, q.difficulty_level, q.total_questions, q.is_published, q.is_ai_generated, q.is_global, q.created_at, q.updated_at`, [parsed.data.isPublished, id, orgId]);
...
quizzesRouter.patch('/:id', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (data.subject !== undefined) {
            ...
            const nextClassLevel = (data.classLevel !== undefined ? data.classLevel : existingRes.rows[0]?.class_level);
            if (nextClassLevel) {
       ...
       RETURNING *, (SELECT title FROM subjects WHERE id = quizzes.subject_id) AS subject`, params);
        if ((result.rowCount ?? 0) === 0) {
            return res.status(404).json({ message: 'Quiz not found' });
        }
        return res.json(result.rows[0]);
...
quizzesRouter.post('/:id/questions', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const quizExists = await db.query('SELECT 1 FROM quizzes WHERE id = $1 AND (organization_id = $2::uuid OR is_global = true)', [id, orgId]);
...
quizzesRouter.post('/attempts', requireAuth, async (req, res) => {
    ...
    try {
        const quizExists = await db.query(`SELECT 1 FROM quizzes WHERE id = $1 AND (organization_id = $2::uuid OR is_global = true)`, [quizId, orgId]);
        if ((quizExists.rowCount ?? 0) === 0) {
            return res.status(404).json({ message: 'Quiz not found' });
        }
        ...
        // again.
        if (idempotencyKey) {
            const existing = await db.query(`SELECT id FROM student_attempts
         WHERE student_id = $1 AND quiz_id = $2 AND idempotency_key = $3
         LIMIT 1`, [studentId, quizId, idempotencyKey]);
        ...
        try {
            await client.query('BEGIN');
            const attemptResult = await client.query(`INSERT INTO student_attempts (student_id, quiz_id, score, total_points, idempotency_key)
         VALUES ($1::uuid, $2::uuid, $3, $4, $5)
         RETURNING id`, [studentId, quizId, score, totalPoints, idempotencyKey]);
            attemptId = attemptResult.rows[0].id;
            for (const qAttempt of questionAttempts) {
                await client.query(`INSERT INTO question_attempts (attempt_id, question_id, is_correct, response_data, time_spent_seconds)
           VALUES ($1::uuid, $2::uuid, $3, $4, $5)`, [attemptId, qAttempt.questionId, qAttempt.isCorrect, qAttempt.responseData || {}, qAttempt.timeSpentSeconds || null]);
            }
            await client.query('COMMIT');
        }
        catch (txError) {
        ...
        try {
            const meta = await db.query(`SELECT q.title AS quiz_title,
