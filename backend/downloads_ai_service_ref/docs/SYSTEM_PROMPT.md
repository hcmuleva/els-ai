...
This is the prompt used for every chat turn (`src/agent/systemPrompt.ts`).
Edit it in the `.ts` file — this copy is for easy reading/review.
...
## Why the "propose, then confirm" split

The model is deliberately never given a tool that generates content
directly. It can only *propose*. This is what makes the Generate/Cancel
button pair meaningful and safe: nothing gets created, and nothing costs an
LLM generation call, until the user explicitly clicks Generate. It also
means a flaky or overeager model response can never silently create content
the user didn't ask for — worst case it shows a wrong proposal card, which
the user just cancels.
