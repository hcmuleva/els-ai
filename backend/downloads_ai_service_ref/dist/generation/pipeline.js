...
import { PIPELINE_STEPS, validateParamsForType } from "./types.js";
function emitStepStart(job, step) {
...
function emitStepComplete(job, step) {
...
export async function runGenerationJob(job) {
    try {
        ...
        if (issues.length > 0) {
            throw Object.assign(new Error(`Generated content failed validation: ${issues.map((i) => i.message).join("; ")}`), { issues });
        }
        await sleep(150);
        ...
        jobStore.update(job.id, {
            status: "completed",
            currentStep: null,
            result: { contentId: result.contentId, name: result.name, preview: result.preview },
        });
