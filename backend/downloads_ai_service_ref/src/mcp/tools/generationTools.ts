...
export const startGenerationTool = {
  ...
  description:
    "Kick off generation for a proposal the user has already confirmed (tapped Generate). " +
    "Returns a jobId to poll or stream progress from — does not block until completion.",
  inputSchema: StartGenerationInput,
  handler: async (input: z.infer<typeof StartGenerationInput>) => {
...
export const reviseGenerationTool = {
  ...
  handler: async (input: z.infer<typeof ReviseGenerationInput>) => {
    const existing = contentRepo.get(input.contentId);
    if (!existing) throw new Error(`Content ${input.contentId} not found`);
...
export const getGenerationStatusTool = {
  ...
  handler: async (input: z.infer<typeof GetGenerationStatusInput>) => {
...
export const searchContentTool = {
  ...
  handler: async (input: z.infer<typeof SearchContentInput>) => {
