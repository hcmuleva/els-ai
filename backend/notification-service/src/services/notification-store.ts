...
function channelFor(organizationId: string, userId: string): string {
  return `notification:${organizationId}:${userId}`;
}

async function pushRealtime(name: string, channel: string, data: Record<string, unknown>) {
  if (eventBus instanceof AblyEventBus) {
    const push: PushChannel = { channel, name, data };
    try {
...
export const NotificationStore = {
  ...
  async aggregateActivity(input: {
    ...
    const existing = await db.query<NotificationRow>(
      `SELECT * FROM notifications
        ...
        LIMIT 1
        FOR UPDATE`,
      [input.userId, input.audienceType, input.windowMinutes, input.classroomId],
    );

    const prevMeta = (existing.rows[0]?.metadata as any) || {};
    const counts: { quiz: number; assignment: number } = {
      quiz: Number(prevMeta?.counts?.quiz || 0),
      assignment: Number(prevMeta?.counts?.assignment || 0),
    };
    if (input.activityKind === 'quiz_submitted') counts.quiz += 1;
    else counts.assignment += 1;

    const studentsSet = new Set<string>(Array.isArray(prevMeta?.students) ? prevMeta.students : []);
    studentsSet.add(input.studentUserId);
    const students = Array.from(studentsSet);

    const segs: string[] = [];
    if (counts.quiz > 0) segs.push(`${counts.quiz} quiz${counts.quiz > 1 ? 'zes' : ''}`);
    if (counts.assignment > 0) segs.push(`${counts.assignment} assignment${counts.assignment > 1 ? 's' : ''}`);
    ...
    if (existing.rows.length > 0) {
      ...
      const updated = await db.query<NotificationRow>(
        `UPDATE notifications
          ...
          WHERE id = $1::uuid
          RETURNING *`,
        [row.id, title, message, JSON.stringify(metadata), input.ctaLabel, input.ctaRoute],
      );
  ...
  async createMany(inputs: CreateNotificationInput[]): Promise<NotificationRow[]> {
  ...
  async listForUser(
    ...
    const countResult = await db.query<{ total: string }>(
      `SELECT COUNT(*)::text AS total FROM notifications WHERE ${where.join(' AND ')}`,
      params,
    );
    params.push(limit, offset);
    const dataResult = await db.query<NotificationRow>(
  ...
  async unreadCount(userId: string): Promise<number> {
    const result = await db.query<{ total: string }>(
      `SELECT COUNT(*)::text AS total FROM notifications
         WHERE user_id = $1::uuid AND status = 'unread' AND deleted_at IS NULL AND expiry_at > NOW()`,
      [userId],
    );
    return Number(result.rows[0]?.total || 0);
  },

  async markRead(userId: string, notificationId: string): Promise<NotificationRow | null> {
    const result = await db.query<NotificationRow>(
      `UPDATE notifications
  ...
  async markAllRead(userId: string, organizationId: string): Promise<number> {
    const result = await db.query(
      `UPDATE notifications SET status = 'read', read_at = NOW()
         WHERE user_id = $1::uuid AND status = 'unread' AND deleted_at IS NULL`,
      [userId],
    );
  ...
  async deleteOne(userId: string, organizationId: string, notificationId: string): Promise<boolean> {
    ...
    try {
      ...
      await client.query(
        `UPDATE notifications SET status = 'read', read_at = COALESCE(read_at, NOW())
           WHERE parent_notification_id = $1::uuid AND deleted_at IS NULL`,
        [row.id],
      );
  ...
  async deleteRange(
    ...
    const cutoffSql: Record<typeof range, string> = {
    ...
    const result = await db.query(
      `UPDATE notifications SET deleted_at = NOW()
         WHERE user_id = $1::uuid AND deleted_at IS NULL
           AND created_at >= ${cutoffSql[range]}`,
      [userId],
    );
  ...
  async deleteAllRead(userId: string, organizationId: string): Promise<number> {
    const result = await db.query(
      `UPDATE notifications SET deleted_at = NOW()
         WHERE user_id = $1::uuid AND deleted_at IS NULL AND status = 'read'`,
      [userId],
    );
    await pushRealtime('notifications_cleared', channelFor(organizationId, userId), { kind: 'read_all' });
    return result.rowCount ?? 0;
  ...
  async expireClassroomCtas(classroomId: string, reason: 'ended' | 'deleted'): Promise<{ rows: NotificationRow[] }> {
    ...
    const result = await db.query<NotificationRow>(
      `UPDATE notifications
       ...
       RETURNING *`,
      [affectedTypes, classroomId, updatedLabel, reason],
    );
  ...
  async teacherActivityCounts(userId: string): Promise<Array<{ classroomId: string; unread: number }>> {
    const result = await db.query<{ classroom_id: string; unread: string }>(
      `SELECT metadata ->> 'classroomId' AS classroom_id,
              SUM(
  ...
  async markTeacherActivitySeen(userId: string, classroomId: string): Promise<number> {
    const result = await db.query(
      `UPDATE notifications
