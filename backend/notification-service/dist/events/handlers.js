...
async function loadUserName(userId) {
    try {
        const result = await db.query(`SELECT first_name, last_name FROM users WHERE id = $1::uuid LIMIT 1`, [userId]);
...
async function scheduleClassroomReminders(args) {
    ...
    for (const row of rows) {
        if (row.fireAt.getTime() < Date.now())
            continue;
        await db.query(`INSERT INTO notification_schedules
...
export async function registerNotificationHandlers() {
    ...
    await eventBus.subscribe('classroom.ended', async (event) => {
        ...
        if (!orgId)
            return;
        await db.query(`UPDATE notification_schedules SET status = 'cancelled'
         WHERE classroom_id = $1::uuid AND status = 'pending'`, [event.payload.classroomId]);
        ...
        if (quizzes.length > 0) {
            ...
            const attemptsResult = await db.query(`SELECT DISTINCT student_id, quiz_id
           FROM student_attempts
          WHERE student_id = ANY($1::uuid[])
            AND quiz_id = ANY($2::uuid[])`, [studentIds, quizIds]);
            for (const row of attemptsResult.rows) {
                const set = attempts.get(row.student_id) ?? new Set();
                set.add(row.quiz_id);
        ...
        if (assignments.length > 0) {
            ...
            const submissionsResult = await db.query(`SELECT DISTINCT student_id, classroom_assignment_id
           FROM classroom_assignment_submissions
          WHERE student_id = ANY($1::uuid[])
            AND classroom_assignment_id = ANY($2::uuid[])`, [studentIds, assignmentIds]);
            for (const row of submissionsResult.rows) {
                const set = submissions.get(row.student_id) ?? new Set();
        ...
        for (const studentId of studentIds) {
            ...
            if (pendingTotal === 0) {
                ...
                if (pendingQuizzes.length > 0)
                    segments.push(`${pendingQuizzes.length} quiz${pendingQuizzes.length > 1 ? 'zes' : ''}`);
                if (pendingAssignments.length > 0)
                    segments.push(`${pendingAssignments.length} assignment${pendingAssignments.length > 1 ? 's' : ''}`);
    ...
    await eventBus.subscribe('classroom.deleted', async (event) => {
    ...
    await eventBus.subscribe('notification.requested', async (event) => {
        if (event.payload && event.payload.kind === 'student_remark') {
            const orgId = event.organizationId;
            if (!orgId)
                return;
    ...
    await eventBus.subscribe('assignment.submitted', async (event) => {
        const orgId = event.organizationId;
        if (!orgId)
            return;
        const data = event.payload;
    ...
    await eventBus.subscribe('quiz.submitted', async (event) => {
        const orgId = event.organizationId;
        if (!orgId)
            return;
        const data = event.payload;
    ...
    await eventBus.subscribe(['billing.invoice.issued', 'billing.invoice.paid'], async (event) => {
        const orgId = event.organizationId;
        if (!orgId)
            return;
        ...
        const inputs = recipients.map((userId) => ({
            userId,
            organizationId: orgId,
    ...
    await eventBus.subscribe('story.live', async (event) => {
        const orgId = event.organizationId;
        if (!orgId)
            return;
        const { storyId, title, classLevel } = event.payload;
        const studentResult = await db.query(`SELECT DISTINCT u.id
         FROM users u
         INNER JOIN user_roles ur ON ur.user_id = u.id AND ur.organization_id = $1::uuid
         INNER JOIN roles r ON r.id = ur.role_id
        WHERE r.role_name = 'student'
          AND ($2::text IS NULL OR u.class_level = $2)`, [orgId, classLevel || null]);
        const studentIds = studentResult.rows.map((r) => r.id);
        if (studentIds.length === 0)
            return;
        const parentResult = await db.query(`SELECT parent_user_id, student_user_id FROM parent_student_links
