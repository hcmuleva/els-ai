...
assignmentsRouter.get('/_meta', requireAuth, async (_req: AuthenticatedRequest, res) => {
  const totals = await db.query<{ count: string }>(`SELECT COUNT(*)::text AS count FROM classroom_assignment_submissions`);
  res.json({
...
assignmentsRouter.post('/classrooms/:classroomId/:assignmentId/submissions', requireAuth, async (req: any, res) => {
  ...
  try {
    ...
    try {
      const meta = await db.query(
        `SELECT c.id AS classroom_id, c.title AS classroom_title, c.created_by AS teacher_id, ca.title AS assignment_title
           FROM classroom_assignments ca
           INNER JOIN classrooms c ON c.id = ca.classroom_id
