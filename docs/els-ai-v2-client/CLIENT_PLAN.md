...
9. ~~Full multi-agent AI router (Factory AI / Copilot / local LLM providers) per Phase 5.~~ ✅ Done, scoped down by decision. Factory's public API turned out to have no chat/completion endpoint (only Droid coding-session/platform-management APIs — confirmed via docs before writing code), and no Copilot credentials exist; user confirmed via `AskUser`: skip both providers for now, build the router abstraction (provider interface, fallback chain, per-role permission allowlist, cost/usage audit log) with Ollama as the sole registered provider, ready for real providers to be added as a one-file drop-in later. Shipped in `backend/ai-service/src/agents/{types,ollamaProvider,router}.ts` + a new `ai_provider_usage` table/`/ai-usage` endpoints in `core-api`. Full detail, verification steps, and two bugs found/fixed during live testing in `PENDING_ITEMS.md` §9.
10. ~~Teacher/Admin AI features requiring new analytics pipelines (weak-student identification, risk detection, forecasts).~~ ✅ Done. Extracted the risk/forecast heuristics that already existed only inside the single-student Growth Trends report into a shared `frontend/src/utils/riskForecast.ts`, then reused them in two new surfaces: the teacher Student Activity list now sorts worst-first with a risk badge, and a brand-new Admin "Analytics" tab shows school-wide risk distribution, an at-risk roster, and a performance-trend forecast (new `GET /quizzes/admin/analytics` in `core-api`). Per `AskUser`: this stays a statistical heuristic (score thresholds + linear-trend projection), not a trained model, until a dedicated analytics/forecasting service exists — the endpoint and shared util are written so that service can be swapped in later without touching any UI. Found and fixed a real scoring-source bug during verification — see `PENDING_ITEMS.md` §10.
11. ~~Telemetry/crash reporting and feature flags infrastructure.~~ ✅ Feature flags done; crash reporting and telemetry explicitly deferred by decision (not requested this round — see `PENDING_ITEMS.md` §11). Shipped a small server-side flag registry (`backend/core-api/src/services/featureFlags/registry.ts`) with per-organization override storage, `GET`/`PATCH /feature-flags`, a shared `useFeatureFlags()`/`useFeatureFlag()` React Query hook, and a new Admin "Feature Flags" toggle UI — demonstrated end-to-end by making the Admin School Analytics tab (item 10, above) itself flaggable, so a school's admin can turn it off without a deploy if the heuristic-based forecast isn't a good fit for them yet.
12. ~~Formal design system package (shared across native + web) with a documented component library.~~ ✅ Done, scoped down by decision. Per `AskUser`: no new primitives (the existing `Card`/`ConfirmModal`/`LatexText`/`ModalHeader`/`PaginationControls`/`SafeScreen` already cover their niches), kept as an organized folder inside the existing app rather than a separate workspace package (one app consumes it, so a package boundary had no payoff yet), documented via a markdown README only (no Storybook), and did a broader hex-literal → token migration sweep across the three largest offending screens now that real git history exists as a safety net. New `frontend/src/components/common/README.md` catalogs every shared component (purpose, full prop table, usage example) plus the token catalog and the "what's intentionally not tokenized" rule (bare white/black, one-off decorative colors). `reports.tsx`/`manage.tsx`/`planner.tsx` had their exact-match `Colors.*`/`RoleColors.*` hex literals replaced with the token reference (202/123/135 substitutions respectively) — see `PENDING_ITEMS.md` §12 for the tooling detail and the self-caught JSX-syntax bug in the first script draft.
...
Explicit ask: Reports (teacher/admin/parent) and Planner's classroom Analytics tab were rendering single-column even on wide laptop/monitor viewports; asked for "professional styles" and dynamic multi-column/multi-row layout "as per needed." Implemented on the 3 screens with genuinely comparable/paired sections — `ParentReports`' two bar charts, `ReportsScreen`'s 4 stat cards + Class Performance/Topic Gaps cards, and `ClassDetailsScreen`'s 3 `MiniBarChart` summaries — using the codebase's existing `useWindowDimensions()` breakpoint convention at a new 1024px threshold. Variable-length lists/tables (Student Activity, Student Breakdown) were deliberately left full-width. Full detail and verification notes in `PENDING_ITEMS.md` §8.

User confirmed "similarly other also" meant surveying other dashboard-style screens for the same issue. Checked `admin.tsx`, `manage.tsx`, `counseling.tsx`, `index.tsx` — none had a comparable stacking problem. Found and fixed one more: `TrendAnalysisTab.tsx` ("Growth Trends" tab, part of the already-flagged Reports module) — a 16-section single-column report; paired 4 more section-groups (Academic Performance+Non-Academic Development, Teacher Feedback+Parent Feedback, Risk Prediction+Benchmark Comparison, Growth Milestones+Future Projection) into responsive rows with the same 1024px pattern, leaving dense/table/hero/already-paired sections untouched. Verified via `tsc --noEmit` (59, 0 new) and live checks with a real-data seeded account (`rahul@els.ai`). Full detail in `PENDING_ITEMS.md` §8. Not yet scoped: whether "professional styles" extends beyond layout into typography/spacing/shadow polish.
