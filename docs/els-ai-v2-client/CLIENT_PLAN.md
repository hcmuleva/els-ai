...
- ~~No accessibility audit has been run on the RN app itself (only `expo-doctor`/`tsc`/`npm audit` were checked previously, which don't cover a11y). WCAG-AA claims can't be made yet.~~ ✅ P1 item 6 completed: `agent-browser a11y --tags wcag2a,wcag2aa` was run against the login screen and every primary authenticated screen for student, teacher, parent, admin, and superadmin. Contrast and prohibited-ARIA findings were fixed app-wide (see §4 item 6 for the full file list); final re-sweep across all 5 roles reports zero color-contrast violations. A handful of unrelated, pre-existing findings (missing `ActivityIndicator`/form labels, one scrollable-region and one skip-nav case, an `aria-hidden-focus` needs-review item) remain deferred — see §4 item 6.
...
6. ~~Accessibility pass: run a real a11y audit (`agent-browser a11y` or native equivalents) against each role's primary screens, fix flagged issues.~~ ✅ Done, in two halves:
   ...
   - Final live re-sweep across all 5 roles: 0 color-contrast violations everywhere reachable. **Deferred (out of scope, pre-existing, flagged not fixed):** `aria-progressbar-name` on `ActivityIndicator` (165 occurrences app-wide), Settings screen's missing form `label`s (3 nodes, all roles), student Reports' `scrollable-region-focusable`, `subject.tsx`'s skip-nav `bypass` finding, an `aria-hidden-focus` "incomplete" (needs-review, not a violation) on admin/teacher screens. None are color-contrast issues; would need a separate scoping/AskUser round if pursued.
