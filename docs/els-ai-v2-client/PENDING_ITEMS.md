...
## 11. P2 item 11 — Telemetry/crash reporting and feature flags infrastructure — feature flags done, rest deferred by decision

Confirmed before building that none of the three exists anywhere in the codebase: no `Sentry`/`Bugsnag`/analytics SDK is imported by any app code (the one `@sentry/node` hit in `node_modules` is a transitive dev-dependency of `eas-cli`, unrelated to the running app), no `ErrorBoundary` exists in the frontend, and no flags mechanism exists at all. Since this item bundles three genuinely separate tooling decisions (which crash-reporting approach, whether to add usage telemetry, which feature-flag approach) each with real external-dependency and secrets-footprint implications, used `AskUser` before writing any code:
1. Crash reporting — **skip for now**, do only feature flags this round.
2. Usage telemetry/analytics — **skip for now**; no concrete requirement is driving it yet, unlike feature flags (which the client plan already ties to a specific need: "rolling out AI features gradually").
3. Feature flags — **build home-grown**: a DB-backed flags table + admin toggle UI + a simple client hook, no new dependencies.
4. (Moot given #1) Crash-reporting scope (frontend/backend/both) — not needed this round.

**What shipped**, all home-grown, no new npm dependencies:
- `backend/core-api/src/services/featureFlags/registry.ts` — a static list of flag definitions (`key`, `label`, `description`, `defaultEnabled`). Adding a new flaggable feature later is a one-line addition here, no migration needed.
- `backend/core-api/src/services/featureFlags/db.ts` — new `feature_flags` table storing only **overrides**: `(organization_id, flag_key, enabled)`. An org with no override row for a key falls back to that key's `defaultEnabled`, so registering a flag never requires a backfill across existing organizations.
- `GET /feature-flags` (any authenticated role — the client-side gating hook needs this for every user a flag might affect, not just admins) returns the merged `{key,label,description,enabled}` list for the caller's org. `PATCH /feature-flags/:key` (admin/superadmin only, scoped to the caller's own org) upserts an override, so a rollout can differ school by school rather than being all-or-nothing. Wired into `core-api`'s `server.ts` (schema bootstrap alongside the existing `ai-chat` one) and proxied through the gateway (`/feature-flags`, `backend/gateway/src/server.ts` + `README.md`).
- `frontend/src/hooks/useFeatureFlags.ts` — `useFeatureFlags()` (React Query, shared cache key `['feature-flags']`, so every consumer shares one fetch) and `useFeatureFlag(key, fallback=true)`. Fails **open** (returns `fallback`) while loading or on error — same "an infra hiccup shouldn't be more restrictive than the code otherwise would" rule already used for the AI provider fallback chain (§9's `agents/router.ts`).
- `frontend/src/components/admin/FeatureFlagsTab.tsx` (new) — a new Admin "Feature Flags" tab listing every registered flag with a `Switch` toggle (styled to match the existing toggle rows in `settings.tsx`), calling the `PATCH` endpoint and refetching the shared query on change so every other screen reading that flag updates immediately, no reload.
- **Demonstrated the mechanism end-to-end** by making the Admin School Analytics tab (item 10) itself the first flaggable feature: `admin.tsx` now calls `useFeatureFlag('admin_school_analytics')` and filters it out of the tab bar (plus redirects away if it was the active tab) when off. Chosen specifically because it's the most recently-shipped, explicitly heuristic-labeled feature — turning it off doesn't break any core workflow, and it directly matches the flag's own description ("turn this off for a school if that isn't a good fit yet").

Verified via `tsc --noEmit` (`core-api` 0, `gateway` 0, frontend steady at the 59-error baseline, 0 new/resolved) and curl: `GET /feature-flags` returns the default-`true` flag; `PATCH` as `super@els.ai` toggles it and the change round-trips on the next `GET`; `PATCH` as `teacher@els.ai` returns 403; unauthenticated `GET` returns 401; `PATCH` with an unknown key returns 404; `PATCH` with a non-boolean body returns 400. Live `agent-browser` check as `super@els.ai`: opened the new Feature Flags tab, toggled the switch off — the Analytics tab tile disappeared from the grid immediately (no reload) — toggled it back on and the tile reappeared; confirmed clean at both 1600×950 and 390×844 (the switch row wraps its description text properly on mobile, no overflow). Restored the flag to its default `enabled: true` state after testing.

**Deferred, by decision:** crash reporting and usage telemetry — no infrastructure was added for either. If revisited, the open decisions are: crash-reporting approach (home-grown log table vs. Sentry, which needs either a SaaS account+key or self-hosted infra) and telemetry approach (home-grown event log vs. a tool like self-hosted PostHog), plus for crash reporting specifically, whether to cover the RN/web frontend (needs a new global `ErrorBoundary`), the backend services, or both.

---

## 12. P2 item 12 — Formal design system package — done, scoped down by decision

Surveyed the existing state before proposing scope: `src/components/common/` already had six shared primitives (`Card`, `ConfirmModal`, `LatexText`, `ModalHeader`, `PaginationControls`, `SafeScreen`) built on `theme/index.ts`'s `Colors`/`RoleColors`/`Typography`/`Radius`/`Shadow`/`Spacing` tokens, but none of it was documented and the tokens weren't consistently used — a grep found 41 files with ad hoc badge/pill style objects and 8 with ad hoc button style objects, plus hundreds of raw hex-literal colors in the three largest screens. Used `AskUser` to settle four independent scope questions before writing anything:
1. New primitives (e.g. a shared `Button`/`Badge`) — **no**, just document what already exists; the existing six components already cover their niches.
2. Package structure — **keep as an organized folder** inside the existing frontend app, not a separate workspace package; there's exactly one app consuming it, so a package boundary (build step, versioning, publish target) had no payoff yet.
3. Migration breadth — **do a broader sweep** across `reports.tsx`/`manage.tsx`/`planner.tsx` (the three largest hex-literal offenders) now that real git history exists as a safety net for a scripted change like this.
4. Documentation tooling — **markdown README only**, no Storybook.

**What shipped:**
- `frontend/src/components/common/README.md` (new) — catalogs the token system (what each of the six token groups contains, why the specific values were chosen, the darkening history from the earlier a11y pass) and every shared component (purpose, full prop table, a copy-paste usage snippet), plus an explicit "what's intentionally not tokenized" rule: bare `#fff`/`#000` (a hex value's semantic role — text color vs. background surface — isn't implied by the RGB value alone) and one-off decorative colors (subject-icon backgrounds, gradient accents) that exist in exactly one place and don't represent a reusable semantic value.
- Hex-literal → token migration on `reports.tsx`, `manage.tsx`, `planner.tsx`: wrote a small Node script (exact case-insensitive hex-value match against the existing `Colors.*`/`RoleColors.*` token values) rather than hand-editing hundreds of literals. Ran it per file, verifying `tsc --noEmit` stayed at the pre-existing 59-error baseline after each: 202 substitutions in `reports.tsx`, 123 in `manage.tsx`, 135 in `planner.tsx` (both `manage.tsx` and `planner.tsx` needed a new `Colors` import added first — neither had touched the theme file before). Deliberately **did not** invent new tokens for the remaining hex literals in `manage.tsx`/`planner.tsx` (~300 combined) that had no existing token match — most are one-off decorative colors specific to a single quiz-type icon or subject, not duplicated semantic values, so forcing them into `Colors.*` would be a false abstraction rather than a real cleanup.

**Bug found and fixed during this item (self-inflicted, caught before it reached `tsc`/review):** the first version of the substitution script naively replaced every quoted hex literal with a bare token reference. That's correct inside an existing JS expression context (object literals, ternaries, arrays — e.g. `color: "#525C6B"` → `color: Colors.textMuted`), but wrong for a hex used directly as a plain JSX attribute value (e.g. `color="#525C6B"` on a lucide icon), where dropping the quotes without adding the required `{}` produces invalid JSX (`color=Colors.textMuted`). First run on `reports.tsx` produced ~90 new parse errors this way. Reverted the file via `git checkout` (safe specifically because real commit history now exists to fall back to) and rewrote the script with a JSX-attribute-aware first pass — it detects the `identifier="#hex"` pattern (equals sign directly touching both the identifier and the opening quote, which reliably distinguishes a bare JSX attribute from a JS comparison/assignment that would have a space there) and rewrites it to `={Token}`, adding the required braces — followed by a second pass for every other already-in-expression-context occurrence. Re-verified clean.

Verified via `tsc --noEmit` (frontend steady at the 59-error baseline throughout, 0 new/resolved — confirmed via direct before/after diff after each file) and live `agent-browser` checks as `teacher@els.ai` at both 1600×950 and 390×844 on all three touched screens (`/planner`, `/manage`, `/reports` → Dashboard tab, including the risk-badged Student Activity list added in item 10 — the surface most affected by the `RoleColors.superadmin`/`Colors.textMuted` substitutions): all render pixel-identical to before, no overflow or broken icons on mobile.
