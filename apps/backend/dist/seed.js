...
export async function initSchemaAndSeed() {
  ...
  `);
    ...
    for (const seedUser of userSeeds) {
        const userInsert = await db.query(`INSERT INTO users(name, email, password, active_role)
       VALUES($1, $2, $3, $4)
       ON CONFLICT(email)
       DO UPDATE SET
         name = EXCLUDED.name,
       ...
       RETURNING id`, [seedUser.name, seedUser.email, seedUser.password, seedUser.activeRole]);
