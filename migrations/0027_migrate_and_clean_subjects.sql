...
-- 3. Updates subject_id references on child tables (content_topics, learning_contents, quizzes, teacher_standard_subjects) to point to the consolidated ANY subject row.
    ...
    FOREACH t IN ARRAY extra_titles LOOP
      ...
      IF any_subject_id IS NOT NULL THEN
        ...
        IF old_subject_ids IS NOT NULL AND ARRAY_LENGTH(old_subject_ids, 1) > 0 THEN
          ...
          -- Re-link teacher_standard_subjects if column subject_id exists
          IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'teacher_standard_subjects' AND column_name = 'subject_id') THEN
